...
const selectAlreadyOptin = async (waNumberList, userId) => {
    const query = 'SELECT wanumber, wastatus FROM `'+TBL_CONTACTS_MASTER+'` WHERE `userid`=? AND `wanumber` in (?) AND DATEDIFF(`authvalidity`, NOW()) > 0';
    const waContacts = waNumberList.map(item => {
        if (/^\+/.test(item.mobileNumber) == true) {
            item.mobileNumber = item.mobileNumber.replace('+', '');
        }
...
const insertIntoSendMaster = async (waNumberList, errorcode, errormessage, createDate, userId, campaignId ) => {
    const queryInsert = 'INSERT INTO `'+TBL_MESSAGE_SENT_MASTER+'` (mobileno, errcode, errdesc, createdt, readstatus, userid, campaignid) VALUES ?;';
    const values = [];
    waNumberList.forEach(item => {
        values.push([item.wanumber, errorcode, errormessage, createDate, 3, userId, campaignId]);
    });
    const result = await wabot_db.query(queryInsert, [values]);
    return result;
}
...
const updateOptinStatus = async (contactList, userId, campaignId, statusCode) => {
    const result = await Promise.all(contactList.map(item => {
        const query = 'UPDATE '+TBL_REQUEST_MASTER+' SET isoptin = ? WHERE `mobileno`= ? AND userid = ? AND campaignid = ? ;';
                    if (/^\+/.test(item.wanumber) == true) {
                        item.wanumber = item.wanumber.replace('+', '');
                    }
        const values = [statusCode, item.wanumber, userId, campaignId];
        return wabot_db.query(query, values);
    }));
    return result;
}
...
const getMobileNumbersToOptin = async (userId, campaignId, batchSize) =>{
    const query = 'SELECT mobileno, wabaurl, accesstoken, optinattempt FROM `'+TBL_REQUEST_MASTER+'` WHERE `userid`= ? AND campaignid = ? AND isoptin = ? limit ?';
