...
const broadCastAPI = async (retry) => {
    try {
        ...
        const [ oneCampaign ] = await templateService.selectCampaignId(1, 0);
        if(!oneCampaign){
       ...
       console.log('\x1b[33m%s\x1b[0m', `OPTIN Campaign ${campaignId} Started | Count: ${result.length} | Batch Size: ${batchSize} \n`);
       //console.log(result);
        ...
        const contacts = await Promise.all(result.map(async waNumber => {
            ...
            return contactService.getOneContact(waNumber.mobileNumber, userId, wabaNumber);
        }));
        ...
        if (contactsToInsert && contactsToInsert.length) {
            await contactService.insertContacts(contactsToInsert, userId, campaignId, wabaNumber);
        }
        ...
        const expiredContacts = await contactService.getExpiredContacts(result, userId, wabaNumber);  
        let waNumberResult = await contactService.getWaNumberFromContacts(result, userId, wabaNumber);
        ...
        if (result && result.length) {
            const optinContacts = await contactService.selectAlreadyOptin(result, userId, wabaNumber);
            ...
            if (optinContactList && optinContactList.length) {
                ...
                console.log('\x1b[36m%s\x1b[0m', `Already Optin Batch Completed\n`);
            ...
            for (let index = 0; index < parseInt(iteratorLength); index++) {
                setTimeout(async () => {
                    try {
                        ...
                        waContactList = await Promise.all(waIdBatch.map(async oneRecord => {
                            try {
                               ...
                               // console.log(error);
                                ...
                                //console.log( waurl, [ oneRecord ], wabaNumber);

                                console.log('\x1b[31m%s\x1b[0m', `Number: ${oneRecord} | WabaNumber: ${wabaNumber} Failed\n`);
                        ...
                        console.log('\x1b[36m%s\x1b[0m', `Optin Batch ${index + 1} Completed | Time: ${new Date().toLocaleString()}\n`);
        ...
        const executeNonWhatsappBatch = async () => {
            try {
                ...
                if(retryOne && retryOne.length && retry < 3){
                    ...
                    if(failedNumbers && failedNumbers.length){
                        ...
                        await contactService.deletefailedRetryTempContacts(failedNumbers, userId, campaignId);
                    }
...
module.exports = cron.schedule('*/30 * * * * *', broadCastAPI, { scheduled: false });