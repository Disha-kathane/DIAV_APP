...
module.exports = cron.schedule(' */20 * * * *', async () => {
    try {
        console.log('Optin Retry cron started');
        ...
        await Promise.all(campaignList.map(async oneCampaign => {
           ...
           // console.log(waurl, authtoken, campaignId);
            const optinMobileNumbers = await contactService.getRetryMobileNumbersToOptin(userId, campaignId, batchSize);
            ...
            await Promise.all(optinMobileNumbers.map(async oneRecord => {
                try {
                    ...
                    if(oneRecord.optinattempt < 5){
                        ...
                        await contactService.insertIntoSendMaster([ oneRecord.mobileno ], errorcode, errormessage, userId, campaignId); 
                        await contactService.deleteInvalidMobileNo([ oneRecord.mobileno ], userId, campaignId);
                        return;  
                    }  
        ...
        console.log('Optin Retry cron stopped');
