...
const optinContactCron = async (retry) => {
    try {
                                ...
                                try {
                                    ...
                                    if (retry <= 2) {
                                        if (parseInt(iteratorLength) == (index + 1)) {
                                            const result = await contactService.getRetryoptinWaNumberFromContacts(); // jo bhi data aayega;
                                            console.log(result);
                                            console.log(retry);
                                            if (result && result.length) {
                                                optinContactCron(++retry);
                                            }
                                        }
                                    }
...
module.exports = cron.schedule(' 0 1 * * *', optinContactCron, {
    scheduled: false
})
