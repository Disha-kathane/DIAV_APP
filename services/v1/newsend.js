...
const {
    ...
    TBL_PAYMENT_GATEWAY_DETAILS,
    TBL_ORDER_DETAILS_MASTER,
    TBL_CATALOG_MASTER,
    TBL_FAILED_CALLBACK_LOGS
...
insertMessageInSentMasterAPI = async (id, botid, userid, mobileno, objMsg, waMessageId, errorCode, errorDesc, readst, msgType, campaignid, direction, wabanumber, wabid, countrycode, rate, billing, pricing_model, submissiontype, bodyContent, profileName, failedDate, next) => {
    try {
        // console.log('insertMessageInSentMasterAPI: ' + mobileno);
        let query;
        let values;
        if (id == 0) {
            query = "INSERT INTO " + TBL_MESSAGE_SENT_MASTER + "(botid,userid,mobileno,wabapayload,messageid,errcode,errdesc,createdt,appid,status,readstatus,messagetype,source,campaignid,direction,contactno,msg_setting_id, countrycode, rate, billing, pricing_model, submissiontype, body_content, profile_name, billingdt,faileddt)" +
                " VALUES(?,?,?,?,?,?,?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?)";
            values = [botid, userid, mobileno, JSON.stringify(objMsg), waMessageId, errorCode, errorDesc, 0, 0, readst, msgType, 1, campaignid, direction, wabanumber, wabid, countrycode, rate, billing, pricing_model, submissiontype, JSON.stringify(bodyContent), profileName, failedDate];

        }
        else {
            query = "INSERT INTO " + TBL_MESSAGE_SENT_MASTER + "(requestid,botid,userid,mobileno,wabapayload,messageid,errcode,errdesc,createdt,appid,status,readstatus,messagetype,campaignid,direction,contactno,msg_setting_id, countrycode, rate, billing, pricing_model, submissiontype, body_content, profile_name, billingdt,faileddt)" +
                " VALUES(?,?,?,?,?,?,?,?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?)";
            values = [id, botid, userid, mobileno, JSON.stringify(objMsg), waMessageId, errorCode, errorDesc, 0, 0, readst, msgType, campaignid, direction, wabanumber, wabid, countrycode, rate, billing, pricing_model, submissiontype, JSON.stringify(bodyContent), profileName, failedDate];
...
fetchTemplate = async (templateName, userId, wabaNumber, next) => {
    try {
        ...
        // let query = "SELECT a.*, b.hsmnamespace,b.mediaurl, b.mediatype FROM ezb_wa_templates AS a, ezb_wa_msg_settings AS b" +
        //     " WHERE a.userid = b.userid AND a.temptitle = ? and a.userid = ? AND b.wanumber LIKE ?";
        // let values = [templateName, userId, '%' + wabaNumber];

        let query = "SELECT a.*, b.hsmnamespace,b.mediaurl, b.mediatype FROM ezb_wa_templates AS a, ezb_wa_msg_settings AS b" +
            " WHERE a.userid = b.userid AND a.temptitle = ? and a.userid = ? AND b.wanumber = ?";
        let values = [templateName, userId, wabaNumber];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
...
customecallbackpayloadflag = async (callback_wanumber, next) => {
    try {
        ...
        let query = "Select custom_callback_payload_flag FROM ezb_wa_msg_settings where wanumber = ?";
        let values = ["+" + callback_wanumber];
...
fetchOtp = async (userid, contactno, next) => {
    try {
        ...
        let query = "SELECT otp FROM ezb_flowbuilder_session WHERE mobileno = ? AND is_session_end = ? AND userid = ? AND isotp = ?";
        let values = [contactno, 0, userid, 1];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
...
unsubkeyword = async (wanumber, next) => {
    try {
        ...
        let query = "Select stopword from ezb_wa_msg_settings where wanumber = ?";
        let values = [wanumber];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
...
insertDataIntoPurchaseMasterEnd = async (userId, mobileNumber, nonCatalogueId, productRetailerId, orderId, rzpOrderId, messageID, paymentReferenceId, transactionId, transactionType, totalAmount, currency, orderStatus, orderFlag, catalogueType, product_image) => {
    try {
        query = 'INSERT INTO ' + TBL_PURCHASE_MASTER + ' (userid, mobileno, non_catalog_id, product_retailer_id, orderid,rzp_orderid, order_detail_messageid, referenceid, transactionid, transaction_type, total_amount, currency, timestamp, order_status, order_flag, catalog_type, product_image) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,?,?,?)';
        values = [userId, mobileNumber, nonCatalogueId, productRetailerId, orderId, rzpOrderId, messageID, paymentReferenceId, transactionId, transactionType, totalAmount, currency, orderStatus, orderFlag, catalogueType, product_image];
        rows = await dbpool.query(query, values);
        return rows[0];
    } catch (err) {
        console.log(err);
...
insertDataIntoPurchaseMaster = async (userId, mobileNumber, nonCatalogueId, productRetailerId, orderId, rzpOrderId, messageID, paymentReferenceId, transactionId, transactionType, totalAmount, currency, orderStatus, orderFlag, catalogueType, productImage, userDetails) => {
    try {
        query = 'INSERT INTO ' + TBL_PURCHASE_MASTER + ' (userid, mobileno, non_catalog_id, product_retailer_id, orderid,rzp_orderid, order_detail_messageid, referenceid, transactionid, transaction_type, total_amount, currency, timestamp, order_status, order_flag, catalog_type, product_image,customer_details) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,?,?,?,?)';
        values = [userId, mobileNumber, nonCatalogueId, productRetailerId, orderId, rzpOrderId, messageID, paymentReferenceId, transactionId, transactionType, totalAmount, currency, orderStatus, orderFlag, catalogueType, productImage, userDetails];
...
updateDataIntoPurchaseMaster = async (paymentReferenceId, transactionId, transactionType, orderStatus, orderFlag, paymentTimestamp, messageID, mobileNumber) => {
    try {
        console.log({ paymentReferenceId, transactionId, transactionType, orderStatus, orderFlag, paymentTimestamp, messageID, mobileNumber });
        query = 'UPDATE ezb_non_catalog_purchase_master SET referenceid = ?, transactionid =?, transaction_type =? , order_status =? , order_flag =?,payment_timestamp = ?, payment_status_message_id = ? WHERE mobileno = ? AND orderid = ?';
        values = [paymentReferenceId, transactionId, transactionType, orderStatus, orderFlag, paymentTimestamp, messageID, mobileNumber, paymentReferenceId];
        rows = await dbpool.query(query, values);
        return rows[0];
    } catch (err) {
        console.log(err);
...
customCallbackPayloadData = async (wanumber) => {
    try {
        ...
        query = 'SELECT * FROM ' + TBL_WA_MSG_SETTINGS_MASTER + ' WHERE wanumber = ?';
        values = ["+" + wanumber];
        rows = await dbpool.query(query, values);
        return rows[0];
    } catch (err) {
...
getRazorpayAuthDetail = async (userId, isRzpLive) => {
    try {
        query = 'SELECT * FROM ' + TBL_PAYMENT_GATEWAY_DETAILS + ' WHERE userid = ? AND is_live = ?';
        values = [userId, isRzpLive];
        rows = await dbpool.query(query, values);
        return rows[0];
    } catch {
        console.log(err);
...
updateDataIntoPurchaseMasterForContactsPayload = async (paymentReferenceId, paymentTransactionId, paymentTransactionType, paymentTotalAmount, paymentCurrency, timestamp, paymentStatus, orderFlag, contactsPaymentMessageId, mobileNumber) => {
    try {
        console.log({ paymentReferenceId, paymentTransactionId, paymentTransactionType, paymentTotalAmount, paymentCurrency, timestamp, paymentStatus, orderFlag, contactsPaymentMessageId, mobileNumber });
        query = 'UPDATE ' + TBL_PURCHASE_MASTER + ' SET referenceid = ?, transactionid =?, transaction_type =? , total_amount =? , currency = ?, timestamp = ?, order_status =? , order_flag =?, contacts_payment_message_id = ? WHERE mobileno = ? AND orderid = ?';
        values = [paymentReferenceId, paymentTransactionId, paymentTransactionType, paymentTotalAmount, paymentCurrency, timestamp, paymentStatus, orderFlag, contactsPaymentMessageId, contactsMobileNumber, paymentReferenceId];
        rows = await dbpool.query(query, values);
        return rows[0];
    } catch (err) {
        console.log(err);
...
fetchFlowIdByMobileNumber = async (mobileNumber, userid, next) => {
    try {
        let query = "SELECT id, current_message_id FROM " + TBL_FLOWBUILDER_SESSION + " WHERE userid = ? AND mobileno = ? AND is_session_end = ?";
        let values = [userid, mobileNumber, 0];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
        console.log('fetchFlowId: ' + err);
...
getTaxData = async (userId, catalogueId, next) => {
    try {
        // console.log('getTaxData ' + userId + '' + catalogueId);
        query = 'SELECT tax FROM ' + TBL_CATALOG_MASTER + ' WHERE userid = ? AND catalogid = ?';
        rows = await dbpool.query(query, [userId, catalogueId]);
        // console.log(JSON.stringify(rows[0]));
        next(null, rows[0][0].tax);
    }
    catch (err) {
        console.log(err);
        next(err);
...
orderDetailsPayload = async (wanumber, orderDetailsPayload, mobileNumber, flowId, current_message_id) => {
    try {
        // console.log('orderDetailsPayload');
        query = 'REPLACE INTO ' + TBL_ORDER_DETAILS_MASTER + '(wanumber, order_details_payload, mobileno, flowid, catalog_node_id) VALUES(?,?,?,?,?)';
        rows = await dbpool.query(query, [wanumber, JSON.stringify(orderDetailsPayload), mobileNumber, flowId, current_message_id]);
        console.log(JSON.stringify(rows[0]));
        return rows[0];
    }
    catch (err) {
...
getOrderDetailsData = async (mobileNumber, flowId, next) => {
    try {
        // console.log('orderDetailsPayload');
        query = 'SELECT * FROM ' + TBL_ORDER_DETAILS_MASTER + ' WHERE mobileno = ? AND flowid= ? LIMIT 1';
        rows = await dbpool.query(query, [mobileNumber, flowId]);
        console.log(JSON.stringify(rows[0]));
        next(null, rows[0]);
    }
    catch (err) {
...
updateOrderDetailsData = async (contextId, mobileNumber, flowId, purchase_master_id) => {
    try {
        // console.log('orderDetailsPayload');
        query = 'UPDATE ' + TBL_ORDER_DETAILS_MASTER + ' SET purchase_master_id = ?, context_message_id = ? WHERE mobileno = ? AND flowid= ?';
        rows = await dbpool.query(query, [purchase_master_id, contextId, mobileNumber, flowId]);
        console.log(JSON.stringify(rows[0]));
        return rows[0];
    }
    catch (err) {
...
getTemplateData = async (templateTitle, userId, next) => {
    try {
        let query = "SELECT * FROM " + TBL_TEMPLATE_MASTER + " WHERE temptitle = ? AND userid = ?";
        let values = [templateTitle, userId];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
        console.log(err);
...
fetchWAPAYContextId = async (mobileNumber) => {
    try {
        query6 = "SELECT session_id FROM ezb_flowbuilder_attributes WHERE attrvalue = 'PAY USING WAPAY' AND flowid =  (SELECT MAX(id) FROM  ezb_flowbuilder_session WHERE mobileno = ? AND is_session_end = 0) ORDER BY id DESC LIMIT 1";
        rows6 = await dbpool.query(query6, [mobileNumber]);
        // console.log("rows6 =========", JSON.stringify(rows6[0]));
        contextId = rows6[0][0].session_id;
        console.log({ contextId });
        return rows6[0][0].session_id;
    }
    catch (err) {
        console.log(err);
        return err;
...
insertFailedCallbackLogs = async (payload, msgId, status, wanumber, customCallbackUrl, retryCount, payloadType) => {
    try {
        console.log('insertFailedCallbackLogs');
        query = 'INSERT INTO ' + TBL_FAILED_CALLBACK_LOGS + ' (payload, messageid,status, wanumber,custom_callback, retry_count, logdate, payload_type,ts) VALUES(?,?,?,?,?,?,CURDATE(),?,NOW())';
        rows = await dbpool.query(query, [JSON.stringify(payload), msgId, status, wanumber, customCallbackUrl, retryCount, payloadType]);
        // console.log(JSON.stringify(rows[0]));
        return rows[0];
    }
    catch (err) {
...
getDataFromFailedCallbackLogs = async () => {
    try {
        // console.log('getDataFromFailedcallbackLogs');
        query = 'SELECT * FROM ' + TBL_FAILED_CALLBACK_LOGS + ' WHERE retry_count < 5 LIMIT 100';
...
getMessageIdCountFromFailedCallbackLogs = async (messageId, readst) => {
    try {
        // console.log('getMessageIdCountFromFailedCallbackLogs');
        query = 'SELECT COUNT(1) AS C FROM ' + TBL_FAILED_CALLBACK_LOGS + ' WHERE messageid = ? AND status = ?';
        rows = await dbpool.query(query, [messageId, readst]);
...
updateRetryPayloadData = async (retryCount, messageId) => {
    try {
        // console.log('updateRetryPayloadData');
        query = 'UPDATE ' + TBL_FAILED_CALLBACK_LOGS + ' SET retry_count = ?, retrydt = NOW() WHERE messageid = ?';
        rows = await dbpool.query(query, [retryCount, messageId]);
        // console.log(JSON.stringify(rows[0]));
        return rows[0];
    } catch (err) {
        console.log(err);
...
deleteRetrySuccessPayloadData = async (messageId) => {
    try {
        // console.log('deleteRetrySuccessPayloadData');
        query = 'DELETE FROM ' + TBL_FAILED_CALLBACK_LOGS + ' WHERE retry_count = ? AND messageid = ?';
        rows = await dbpool.query(query, [0, messageId]);
        // console.log(JSON.stringify(rows[0]));
        return rows[0];
    } catch (err) {
        console.log(err);
...
getCustomCallbackUrl = async (wanumber) => {
    try {
        // console.log('getCustomCallbackUrl' + wanumber);
        query = 'SELECT custom_callback FROM ' + TBL_WA_MSG_SETTINGS_MASTER + ' WHERE wanumber = ?';
        rows = await dbpool.query(query, ['+' + wanumber]);
        // console.log(JSON.stringify(rows[0]));
        return rows[0][0].custom_callback;
    }
    catch (err) {
        console.log(err);
        return err;
...
setInitChatAttributes = async (flowid, current_message_id, attrkey, attrvalue, session_id, mobileno, messageContent, messageType, payload, next) => {
    // console.log('setChatAttributes query=================>' + JSON.stringify(payload));
    try {
        let is_placeholder = 0;
        let is_validator = 0;
...
checkMobileNumber = async(mobileno)=>{
    try {
        query = 'SELECT COUNT(1) AS C FROM ezb_diav_member_master WHERE mobileno = ?';
        rows = await dbpool.query(query, [mobileno]);
        // console.log(JSON.stringify(rows[0][0].C));
        return rows[0][0].C;
    } catch (err) {
        console.log(err);
...
module.exports = {
    ...
    getRazorpayAuthDetail,
    getPaymentDetails,
    updateDataIntoPurchaseMasterForContactsPayload,
    fetchFlowIdByMobileNumber,
    getTaxData,
    orderDetailsPayload,
    getOrderDetailsData,
    updateOrderDetailsData,
    insertDataIntoPurchaseMasterEnd,
    getTemplateData,
    fetchWAPAYContextId,
    insertFailedCallbackLogs,
    getDataFromFailedCallbackLogs,
    getMessageIdCountFromFailedCallbackLogs,
    updateRetryPayloadData,
    deleteRetrySuccessPayloadData,
    getCustomCallbackUrl,
    setInitChatAttributes,
    storeDIAVOTP,
    checkDIAVOTP,
    checkMobileNumber
}
