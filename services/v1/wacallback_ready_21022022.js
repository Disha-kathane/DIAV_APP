...
updateDelivery = async (obj, userId, next) => {
    try {
        ...
        if (status == 'sent' || status == 'delivered' || status == 'read') {
            ...
            if (status == 'sent') {
                query = "UPDATE " + TBL_MESSAGE_SENT_MASTER + " SET readstatus = ?, sessid = ?, sentdt = ?, billing = ?, pricing_model = ?, submissiontype = ?, countrycode = ?, rate = ?, billingdt = IF(billingdt IS NULL, ?, billingdt), category = ?, expiration_timestamp = ?, category_id = ?, errcode = NULL, errdesc = NULL WHERE messageid = ? AND rate IS NULL";
                ...
                query = "UPDATE " + TBL_MESSAGE_SENT_MASTER + " SET readstatus = ?, sessid = ?, dlvrddt = ?, billing = ?, pricing_model = ?, submissiontype = ?, countrycode = ?, rate = ?, billingdt = IF(billingdt IS NULL, ?, billingdt), category = ?, category_id = ?, errcode = NULL, errdesc = NULL WHERE messageid = ? AND rate IS NULL";
                ...
                query = "UPDATE " + TBL_MESSAGE_SENT_MASTER + " SET readstatus = ?, sessid = ?, readdt = ?, billing = ?, pricing_model = ?, submissiontype = ?, countrycode = ?, rate = ?, billingdt = IF(billingdt IS NULL, ?, billingdt), category = ?, category_id = ?, errcode = NULL, errdesc = NULL, dlvrddt = IF(dlvrddt IS NULL, ?, dlvrddt) WHERE messageid = ? AND rate IS NULL";
                ...
                query = "UPDATE " + TBL_MESSAGE_SENT_MASTER + " SET readstatus = ?, readdt = ?, billingdt = IF(billingdt IS NULL, ?, billingdt), errcode = NULL, errdesc = NULL, dlvrddt = IF(dlvrddt IS NULL, ?, dlvrddt) WHERE messageid = ?";
...
updateDelivery_02032022 = async (obj, userId, next) => {
    try {
        let query;
        let rows;
        let values;
...
updateDelivery_18022022_old = async (obj, userId, next) => {
    try {
        let query;
        let rows;
        let values;
...
updateDelivery_OldPricing = async (obj, userId, next) => {
    try {
        ...
        if (status == 'delivered') {
            ...
            if (obj.statuses[0].conversation != undefined && obj.statuses[0].conversation.id != undefined) {
                sessid = obj.statuses[0].conversation.id; chargeon_flag_api
                billing = obj.statuses[0].pricing.billable;
                submissiontype = billing == true ? "NOTIFICATION" : "FEEDBACK";
                pricing_model = obj.statuses[0].pricing.pricing_model;
                recipient_id = obj.statuses[0].recipient_id;
