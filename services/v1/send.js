...
updateMediaChatAttributes = async (session_id, attrval, mediatype, mediaid, mediaurl, next) => {
    try {
        let query = "UPDATE " + TBL_FLOWBUILDER_ATTRIBUTE + " SET attrvalue = ?, mediatype = ?, mediaid = ?, mediaurl = ? WHERE session_id = ?";
        let values = [attrval, mediatype, mediaid, mediaurl, session_id];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
        console.log(err);
...
fetchPlaceholderValue = async (mobileno, attrkey, userId, next) => {
    try {
        ...
        let query = "SELECT attrvalue FROM " + TBL_FLOWBUILDER_ATTRIBUTE + " WHERE id =" +
            ...
            " WHERE a.message_type IN('text','interactive') AND a.attrkey = ? AND a.session_mobile = ?" +
...
fetchWebhookPlaceholderValue = async (mobileno, attrkey, userId, next) => {
    try {
        // let query = "SELECT attrvalue FROM " + TBL_FLOWBUILDER_ATTRIBUTE + " WHERE id =" +
        ...
        let query = "SELECT attrvalue FROM " + TBL_FLOWBUILDER_ATTRIBUTE + " WHERE id =" +
            "(SELECT MAX(a.id) FROM " + TBL_FLOWBUILDER_ATTRIBUTE + " AS a," + TBL_FLOWBUILDER_SESSION + " AS b" +
            " WHERE a.message_type IN('webhook') AND a.attrkey = ? AND a.session_mobile = ?" +
            " AND a.flowid = b.id AND b.userid = ? AND b.is_session_end IN (?,?))";
...
updateOtp = async (otp, contactno, id, next) => {
    try {
        let query = "UPDATE ezb_flowbuilder_session SET isotp = ?, otp = ? WHERE id = ?";
        let values = [1, otp, id];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
...
fetchFlowSessionId = async (contactno, next) => {
    try {
        let query = "SELECT MAX(id) AS id FROM ezb_flowbuilder_session WHERE mobileno LIKE ? AND is_session_end = ?";
        let values = ['%' + contactno, 0];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
        console.log(err);
...
fetchWabaNumber = async (apikey, next) => {
    try {
...
fetchWabaTemplate = async (apikey, next) => {
    try {
