...
fetchFlowSession = async (mobileno, userId, next) => {
    try {
        ...
        let query = "SELECT a.mobileno, a.id, COUNT(1) AS session, a.flowid, a.next_message_id, a.current_message_id, a.is_node_option, a.current_message_type," +
            ...
            " AND a.id = b.flowid AND a.userid = ?";
...
fetchNextNodeOption = async (flowID, inputText, next) => {
    try {
...
isFlowChatKeyword = async (searchKeyword, next) => {
    try {
        // let query = "SELECT count(1) AS c FROM " + TBL_FLOWBUILDER_MASTER + " WHERE LOWER(keywords) RLIKE LOWER(?)";
        // let values = ['[[:<:]]' + searchKeyword + '[[:>:]]'];
        let query = "SELECT count(1) AS c FROM ezb_flowbuilder_master WHERE FIND_IN_SET(LOWER(?),LOWER(keywords))";
        let values = [searchKeyword];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    } catch (err) {
...
checkConditionInNodeOption = async (flowID, inputText, next) => {
    try {
        let query = "SELECT a.id, a.next_message_id, a.node_body, a.typenode, a.placeholder, a.is_placeholder, a.validator, a.is_validator," +
            " a.error_message, a.is_webhook, a.webhook FROM ezb_flowbuilder_node AS a, ezb_flowbuilder_node_option AS b, ezb_flowbuilder_session AS c" +
...
fetchNextNodeOptionWebhook = async (inputText, next) => {
    try {
...
fetchLiveChatSettings = async (mobileno, next) => {
    try {
        let query = "SELECT a.live_chat_webhook, a.live_chat_token, COUNT(1) AS c FROM ezb_flowbuilder_master AS a, ezb_flowbuilder_session AS b" +
            " WHERE a.flowid = b.flowid AND b.mobileno = ? and b.is_livechat = ? AND b.is_session_end = ?";
        let values = [mobileno, 1, 0];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    }
    catch (err) {
...
fetchEndLiveChatMessage = async (mobileno, text, next) => {
    try {
        let query = "SELECT a.end_chat_message, COUNT(1) AS c FROM ezb_flowbuilder_master AS a, ezb_flowbuilder_session AS b" +
            " WHERE a.flowid = b.flowid AND b.mobileno = ? AND b.is_livechat = ? AND b.is_session_end = ? AND LOWER(end_chat_keyword)= LOWER(?)";
        let values = [mobileno, 1, 0, text];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    }
    catch (err) {
...
endLiveChatMessage = async (mobileno, next) => {
    try {
        let query = "UPDATE ezb_flowbuilder_session SET is_session_end = ? WHERE is_livechat = ? AND mobileno = ? AND is_session_end = ?";
        let values = [1, 1, mobileno, 0];
        let rows = await dbpool.query(query, values);
        next(null, rows[0]);
    }
    catch (err) {
...
checkLiveChatStatus = async (mobileno, next) => {
    try {
