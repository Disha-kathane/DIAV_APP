...
insertMessageInSentMaster = async (id, botid, userid, mobileno, objMsg, waMessageId, msgType, campaignid, direction, wabanumber, wabid, bodyContent, next) => {
    try {
        ...
        let values;
        if (id == 0) {
            query = "INSERT INTO " + TBL_MESSAGE_SENT_MASTER + "(requestid,botid,userid,mobileno,wabapayload,messageid,errcode,errdesc,createdt,appid,status,readstatus,messagetype,source,campaignid,direction,contactno,msg_setting_id,body_content)" +
            ...
            query = "INSERT INTO " + TBL_MESSAGE_SENT_MASTER + "(requestid,botid,userid,mobileno,wabapayload,messageid,errcode,errdesc,createdt,appid,status,readstatus,messagetype,campaignid,direction,contactno,msg_setting_id,body_content)" +
        ...
        let rows = await dbpool.query(query, values);
        console.log('insertMessageInSentMaster result =======================>' + JSON.stringify(rows));
        next(null, rows[0]);
    }
    catch (err) {
        console.log('insertMessageInSentMaster error =======================>' + JSON.stringify(err));
        next(err);
    }
}
...
insertMessageInSentMasterAPI = async (id, botid, userid, mobileno, objMsg, waMessageId, msgType, campaignid, direction, wabanumber, wabid, countrycode, rate, billing, pricing_model, submissiontype, bodyContent, profileName, next) => {
    try {
        ...
        let values;
        if (id == 0) {
            query = "INSERT INTO " + TBL_MESSAGE_SENT_MASTER + "(requestid,botid,userid,mobileno,wabapayload,messageid,errcode,errdesc,createdt,appid,status,readstatus,messagetype,source,campaignid,direction,contactno,msg_setting_id, countrycode, rate, billing, pricing_model, submissiontype, body_content, profile_name)" +
                " VALUES(((SELECT MAX( requestid ) FROM ezb_message_sent_master C) + 1),?,?,?,?,?,NULL,NULL,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            query = "INSERT INTO " + TBL_MESSAGE_SENT_MASTER + "(requestid,botid,userid,mobileno,wabapayload,messageid,errcode,errdesc,createdt,appid,status,readstatus,messagetype,campaignid,direction,contactno,msg_setting_id, countrycode, rate, billing, pricing_model, submissiontype, body_content, profile_name)" +
                " VALUES(?,?,?,?,?,?,NULL,NULL,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
setFlowSession = async (mobileno, flowID, userId, next_message_id, current_message_id, type, payload, placeholder, next) => {
    try {
        ...
        let query = "INSERT INTO " + TBL_FLOWBUILDER_SESSION + "(flowid,userid,mobileno,next_message_id,current_message_id,current_message_type,session_createdt,is_session_created,session_enddt,is_session_end,is_node_option,is_placeholder,placeholder,is_variant)" +
            " VALUES(?,?,?,?,?,?,NOW(),1,NULL,0,?,?,?,?)";
        let values = [flowID, userId, mobileno, next_message_id, current_message_id, type, is_node_option, is_placeholder, placeholderkey, is_node_option];
        let rows = await dbpool.query(query, values);
        next(null, rows[0].insertId);
    } catch (err) {
        console.log('setFlowSession---------------------->' + err);
        next(err);
    }
}
...
updateNextMessageIdInSession = async (id, next_message_id, current_message_id, is_node_option, type, placeholder, next) => {
    try {
        ...
        console.log('CONDITION ============================>' + id, next_message_id, current_message_id, is_node_option, type, placeholder);

        if (next_message_id > 0) {
            console.log('I AM IN NEXT MESSAGE ID IF BLOCK');
            query = "UPDATE " + TBL_FLOWBUILDER_SESSION + " SET next_message_id = ?, current_message_id = ?, is_node_option = ?, current_message_type = ?, is_placeholder = ?, placeholder = ?, is_variant = ? WHERE id = ?";
            values = [next_message_id, current_message_id, is_node_option, type, is_placeholder, placeholderkey, is_node_option, id];
        }
        else {
            console.log('I AM IN NEXT MESSAGE ID ELSE BLOCK');
            query = "UPDATE " + TBL_FLOWBUILDER_SESSION + " SET next_message_id = ?, current_message_id = ?, is_node_option = ?, current_message_type = ?, is_session_end = ?, session_enddt = NOW(), is_placeholder = ?, placeholder = ?, is_variant = ? WHERE id = ?";
            values = [next_message_id, current_message_id, is_node_option, type, 1, is_placeholder, placeholderkey, is_node_option, id];
        }
        let rows = await dbpool.query(query, values);
        console.log('UPDATE NEXT MESSAGE ID : ' + JSON.stringify(rows));
        next(null, rows[0]);
    }
    catch (err) {
        console.log('updateNextMessageIdInSession---------------->' + err);
        next(err);
    }
}
...
setChatAttributes = async (flowid, current_message_id, attrkey, attrvalue, session_id, mobileno, messageContent, messageType, payload, next) => {
    ...
    try {
        ...
        console.log('setChatAttributes----------------->' + err);
        next(err);
    }
}
...
fetchLastTextMessage = async (mobileno, next) => {
    try {
...
updatePreviousButtonNextMessageIdInSession = async (id, next_message_id, current_message_id, is_node_option, type, placeholder, next) => {
    try {
        ...
        if (next_message_id > 0) {
            query = "UPDATE " + TBL_FLOWBUILDER_SESSION + " SET next_message_id = ?, current_message_id = ?, is_node_option = ?, current_message_type = ?, is_placeholder = ?, placeholder = ?, is_variant = ?, is_session_end = ? WHERE id = ?";
            values = [next_message_id, current_message_id, is_node_option, type, is_placeholder, placeholderkey, is_node_option, 0, id];
        }
        else {
            query = "UPDATE " + TBL_FLOWBUILDER_SESSION + " SET next_message_id = ?, current_message_id = ?, is_node_option = ?, current_message_type = ?, is_session_end = ?, session_enddt = NOW(), is_placeholder = ?, placeholder = ?, is_variant = ? WHERE id = ?";
            values = [next_message_id, current_message_id, is_node_option, type, 1, is_placeholder, placeholderkey, is_node_option, id];
        ...
        console.log('updatePreviousButtonNextMessageIdInSession ------------------>' + err);
        next(err);
    }
}
