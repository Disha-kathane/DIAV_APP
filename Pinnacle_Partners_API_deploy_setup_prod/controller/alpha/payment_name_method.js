...
let paymentname = async function (req, res) {
    try {
        ...
        console.log("paymentmethod apikeyResult -------------------> ", apikeyResult, apikeys, wanumber);
        ...
        if (getWabaIdResult != null && getWabaIdResult.length > 0) {
            ...
            console.log("paymentmethod wabaid -------------------> ", wabaid);
        } else {
            return res.send({
                code: 'WA006',
                status: 'FAILED',
        ...
        console.log("paymentmethod getpaymentname --------------->  ", getpaymentname);
