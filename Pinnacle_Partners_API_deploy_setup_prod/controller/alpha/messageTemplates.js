    ...
    try {
        ...
        let createpayload = function (access_token, wabaid, userid, callback) {
            if (ComponentsData.length > 0) {
                ...
                if (ComponentsData.findIndex(x => x.type === "limited_time_offer") > -1) {
                    console.log("inside limited_time_offer");
                    let LTOindex = ComponentsData.findIndex(x => x.type === "limited_time_offer"); //lto template
                    components.push({
                ...
                if (ComponentsData.findIndex(x => x.type === "BUTTONS") > -1) {
                    ...
                    for (let i = 0; i < urldata.length; i++) {
                        ...
                        if (urldata[i].type === "PHONE_NUMBER") {
                            button_option_string.push({
                                "call_phone": {
                ...
                if (ComponentsData.findIndex(x => x.type === "HEADER") > -1) {
                    ...
                    // let imageindex = ComponentsData.findIndex(x => x.format === "IMAGE")
                    ...
                    // let videoindex = ComponentsData.findIndex(x => x.format === "VIDEO")
                    // let documentindex = ComponentsData.findIndex(x => x.format === "DOCUMENT")
                    // let locationindex = ComponentsData.findIndex(x => x.format === "LOCATION")
        ...
        let createtemplate = function (objData, data, callback) {
            ...
            whatsappService.messageTemplate(objData, wabaid, (err, result) => {
                if (err) {
                    ...
                    // console.log("data inserted ",data.bodyplaceholders, data.urlplaceholder, data.headerplaceholder)
                    ...
                    userService.insertTemplateDetails(result, data, (err, insertTemplateresult) => {
                        if (err) {
                            console.log(err);
                        } else {
