...
updateDelivery_3003 = async (job, next) => {
    ...
    try {
        ...
        if (status === 'sent' || status === 'delivered' || status === 'read') {
            ...
            if (job.data.entry[0].changes[0].value.statuses[0].conversation !== undefined && job.data.entry[0].changes[0].value.statuses[0].conversation.id !== undefined) {
                ...
                console.log({ businessInitiatedRate, userInitiatedRate, responseRate, pinnacleBusinessInitiatedRate, pinnacleUserInitiatedRate, outgoingRate, finalBICRate, finalUICRate, finalRate, free_conversation, is_free_conversation, billing_type, account_type, isSessIdExists });
                    ...
                    if (isSessIdExists === 0 && free_conversation > 0) {
                        ...
                        if (account_type === 1) {
                            ...
                            if (isSessIdExists === 0 && billing_type == 2 && outgoingRate !== null && free_conversation !== 0) {
                                // console.log('======================================================> ' + userId + ' ' + outgoingRate + ' ' + recipient_id);
                                finalRate = outgoingRate;
                                // console.log({ outgoingRate, finalRate });
...
// stable cloud update delivery for pricing update dated 1st June 2023 --- deployed By Vaibhav Dongre
updateDeliveryCloud = async (job) => {
    ...
    try {
        ...
        if (status === 'sent' || status === 'delivered' || status === 'read') {
            ...
            if (job.data.entry[0].changes[0].value.statuses[0].conversation !== undefined && job.data.entry[0].changes[0].value.statuses[0].conversation.id !== undefined) {
                ...
                console.log({ finalMarketing_rate, finalUtility_rate, finalAuthentication_rate, finalService_rate, feedback_rate, finalRate, free_conversation, is_free_conversation, isSessIdExists, balance_amt, account_type, billing_type });
