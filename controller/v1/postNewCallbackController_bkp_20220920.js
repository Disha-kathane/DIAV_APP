...
module.exports = (req, res) => {
        ...
        if (wanumber != null) {
            callbackService.validatewanumber(wanumber, (err, result) => {
                if (result != undefined && result.length > 0) {
                    ...
                    //console.log('validatephonenumber error ==============================>' + JSON.stringify(err));
                    done('User not found');
                }
            });
        } else {
        ...
        if (Object.keys(obj).length > 0) {
            if (wanumber != null && obj.changes[0].value.hasOwnProperty('contacts') &&
                ...
                if (obj.contacts != undefined && obj.messages != undefined) {
                    ...
                    if (bodyText != undefined && bodyText != '' && bodyText.length > 0) {
                        //from here
                            ...
                            if (isLiveChatActive == 0) {
                                async.waterfall([
                                    ...
                                    function (msg_Type, callback) {
                                        if (msg_Type == 1) {
                                            sendService.updateChatAttributes(obj.messages[0].context.id, bodyText, (err, result) => {
                                                if (err) {
                                                    console.log('updateChatAttributes_1 err(' + contactno + ')================================>' + err)
                                                } else {
                                                    console.log('updateChatAttributes_1 result(' + contactno + ')================================>' + JSON.stringify(result))
                                                }
                                    ...
                                    function (flowType, flowTmpPayload, next_message_id, current_message_id, sessionResult, placeholder, nextMessageResult, callback) {
                                        ...
                                        createPayload(userId, contactno, flowType, flowTmpPayload, next_message_id, current_message_id, sessionResult, placeholder, nextMessageResult, obj, isLiveChatActive, bodyText, -1, (err, createPayloadResult) => {
                                            sendEmail(userId, obj, sessionResult);
                                    ...
                                    if (err) {
                                        ...
                                        if (err != null && err.code == 404) {
                                            callbackService.getReplyMessage(bodyText, wanumber, (err, result) => {
                                                ...
                                                if (bodyText.toLowerCase() == unSubscribeKeyword.toString().toLowerCase() && unSubscribeMessage.length > 0) {
                                                    callbackService.updateSubscription(contactno, 0, wanumber, null, bodyText, (err, result) => {
                                                        if (err) {
                                                            console.log('updateSubscription(stop) error==================>' + JSON.stringify(err));
                                                        } else {
                                                            console.log('updateSubscription(stop) result================>' + JSON.stringify(result));
                                                        }
                                                        ...
                                                        logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult1) => {
                                                            if (err) {
                                                                console.log('logSentByUserMessage1 error==================>' + JSON.stringify(err));
                                                            } else {
                                                    ...
                                                    callbackService.updateSubscription(contactno, 1, wanumber, bodyText, null, (err, result) => {
                                                        if (err) {
                                                            console.log('updateSubscription(start) error==================>' + JSON.stringify(err));
                                                        } else {
                                                        ...
                                                        logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult2) => {
                                                            if (err) {
                                                                console.log('logSentByUserMessage2 error==================>' + JSON.stringify(err));
                                                            } else {
                                                    ...
                                                    logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult3) => {
                                                        if (err) {
                                                            console.log('logSentByUserMessage3 error==================>' + JSON.stringify(err));
                                                        } else {
                                            ...
                                            logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult4) => {
                                                if (err) {
                                                    console.log('logSentByUserMessage4 error==================>' + JSON.stringify(err));
                                                } else {
                                            ...
                                            callNextNode(processMessageFlowId, userId, contactno, 2, (err, callNextNodeResult) => {
                                                createPayload(userId, contactno,
                                                    ...
                                                    isLiveChatActive,
                                                    bodyText,
                                                    callNextNodeResult.type, (err, createPayloadResult) => {
                                                        done(null, 'Success');
                                                    });
                                            });
                                        }
                                ...
                                logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult5) => {
                                    if (err) {
                                        console.log('logSentByUserMessage5 error==================>' + JSON.stringify(err));
                                    } else {
                                        console.log('logSentByUserMessage5 result================>' + JSON.stringify(logSentByUserMessageResult5));
                                    }
                                    sendEmail(userId, obj, result);
        ...
        async.waterfall([
            (templateMessageCallback) => {
                sendService.fetchTemplate(autoResponseTemplateName, userId, wabaNumber, (err, templateMessageResult) => {
                    if (err) {
                        console.log('fetchTemplate error========================>' + JSON.stringify(err));
                    } else {
            ...
            if (err) {
                ...
                next(null, result);
                errorLogger.info('SEND TEMPLATE MESSAGE=========================================>' + autoResponseTemplateName + ', TO : ' + to + ', TYPE : ' + type + ', MESSAGEID : ' + result);
            }
        });
    }
                ...
                function (filedata, done) {
                    if (msgType == 0 || msgType == 1 || msgType == 2 || msgType == 3 || msgType == 7 || msgType == 10) {
                        ...
                        s3.upload(params, function (err, data) {
                            if (err) {
                                console.log('Upload Media Error (' + waRecipientNo + ')===============================>' + error);
                                return done(error);
                            } else {
    ...
    let createPayload = (userId, contactno, flowType, flowTmpPayload, next_message_id, current_message_id, sessionResult, placeholder, nextMessageResult, obj, isLiveChatActive, bodyText, mType, next) => {
        async.waterfall([
            (createPayloadCallback) => {
                logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, result) => {
                    console.log(isLiveChatActive);
                    if (err) {
                        console.log('logSentByUserMessage error (' + contactno + ')==================>' + JSON.stringify(err));
                    } else {
            ...
            (flowTmpPayload, botMessagePayload, sessionResult, next_message_id, current_message_id, flowType, typeNode, placeholder, nextMessageResult, createPayloadCallback) => {
                ...
                if (typeNode == "Condition") {
                    ...
                    if (typeNode != "Message") {
                        sendMessage(userId, result.payload, contactno, flowType, (err, sendMessageResult) => {
                            if (err) {
            ...
            (sendMessageResult, saveChatAttributesresult, _result, typeNode, createPayloadCallback) => {
                ...
                if (sendMessageResult != null) {
                    ...
                    webhook(userId, _result, saveChatAttributesresult, obj, (err, webhookResult) => {
                        if (err) {
                            console.log('webhook error==================>' + JSON.stringify(err));
                            ...
                            console.log('webhook result================>' + JSON.stringify(webhookResult));
                            ...
                            if (webhookResult != undefined && webhookResult != null) {
                                createPayload(userId, contactno,
                                    ...
                                    isLiveChatActive,
                                    bodyText,
                                    webhookResult.type, (err, createPayloadResult) => {
                        ...
                        if (value.if_text.toLowerCase() == bodyText.toLowerCase()) {
                            multiCondition(_result, value, contactno, bodyText, obj, (err, multiConditionResult) => {
                                createPayload(userId, contactno,
                                    ...
                                    isLiveChatActive,
                                    bodyText,
                                    multiConditionResult.type, (err, createPayloadResult) => {
                                        createPayloadCallback(err, createPayloadResult);
                                    });
                            });
                        } else {
                    ...
                    checkCondition(_result, bodyText, contactno, obj, (err, checkConditionResult) => {
                        if (err) {
                            console.log('checkCondition error==================>' + JSON.stringify(err));
                        } else {
                            console.log('checkCondition result================>' + JSON.stringify(checkConditionResult));
                        }
                        console.log('checkCondition result================>' + contactno,
                        ...
                        createPayload(userId, contactno,
                            ...
                            isLiveChatActive,
                            bodyText,
                            checkConditionResult.type, (err, createPayloadResult) => {
                                createPayloadCallback(err, createPayloadResult);
                            });
                    })
                } else {
            ...
            //     logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, result) => {
            //         if (err) {
            //             console.log('logSentByUserMessage error==================>' + JSON.stringify(err));
            //         } else {
            ...
            async.waterfall([
                (d1) => {
                    ...
                    sendService.fetchPlaceholderValue(contactno, tmpKey, userId, (err, result) => {
                        if (result.length > 0 && result[0].attrvalue != null) {
    ...
    let executeWebhook = (userId, contactno, webhookPayload, next) => {
        async.waterfall([
            function (webHookCb) {
                ...
                if (webHookAttrKeyArr != null && webHookAttrKeyArr.length > 0) {
                    replacePlaceholder(userId, tmpWebObj, webHookAttrKeyArr, contactno, userProfileName, userMobileNumber, (err, result) => {
                        //console.log('executeWebhookUrl==========================>' + result);
            ...
            function (webHookApi, webHookCb) {
                ...
                axios(config)
                    .then(function (webhookresponse) {
                        console.log('EXECUTE_WEBHOOK_RESULT==============================>' + JSON.stringify(webhookresponse.data));
                        webHookCb(null, webhookresponse.data);
                    })
                    .catch(function (error) {
                        console.log('EXECUTE_WEBHOOK_ERROR==============================>' + error);
                        webHookCb(error);
                    });
            }
        ], (err, result) => {
    ...
    let callCustomCallback = (url, custom_parameters, obj) => {
        //console.log('Custom Callback Payload====================================>' + JSON.stringify(obj));
        let data = null;
        let callback_wanumber = obj.entry[0].changes[0].value.metadata.display_phone_number;
        ...
        async.waterfall([
            function (done) {
                sendService.customecallbackpayloadflag(callback_wanumber, (err, customecallbackpayloadflagResult) => {
                    if (err) {
                        console.log('customecallbackpayloadflag err=======================>' + JSON.stringify(err));
                        done(err);
                    } else {
    ...
    let sendEmail = (userId, obj, result) => {
        async.waterfall([
            function (sendEmailCallback) {
                console.log('SEND_EMAIL=====================>' + JSON.stringify(result));
                sendService.fetchEmailData(result.next_message_id, (err, fetchEmailDataResult) => {
            ...
            function (fetchEmailDataResult, sendEmailCallback) {
                ...
                if (attrKeyArr != null && attrKeyArr.length > 0) {
                    ...
                    replacePlaceholder(userId, tmpEmailObj, attrKeyArr, obj.contacts[0].wa_id, userProfileName, userMobileNumber, (err, result) => {
    ...
    let webhook = (userId, result, chatAttrInsertId, obj, next) => {
        async.waterfall([
            function (webhookCallback) {
                executeWebhook(userId, obj.contacts[0].wa_id, result, (err, executeWebhookResult) => {
            ...
            function (result, chatAttrInsertId, executeWebhookResult, webhookCallback) {
                ...
                if (tmpWebhookResCodeArr.length > 0) {
                    ...
                    if (tmpWebhookResCodeArr.includes(tmpWebhookResCode)) {
                        sendService.fetchNextNodeOptionWebhook(tmpWebhookResCode, obj.contacts[0].wa_id, (err, fetchNextNodeOptionWebhookResult) => {
                            if (err) {
