...
const { paymentOption, paymentOptionForCatalogue } = require('../../services/v1/payment');
...
module.exports = (req, res) => {
    ...
    let readobj = (done) => {
        ...
        else if (body.object === 'whatsapp_business_account' && req.body.entry[0].changes[0].field == 'template_category_update') {
        ...
        else if (body.object === 'whatsapp_business_account' && req.body.entry[0].changes[0].field == 'template_performance_metrics') {
    ...
    let validatewanumber = (obj, done) => {
        ...
        else if (obj.changes[0].field == 'template_category_update') {
            sendService.getWaSettingsForTemplateStatus(msgtempid, (err, result) => {
                console.log('getWaSettingsForTemplateStatus================>' + JSON.stringify(result));
                if (result.length > 0) {
                    done(null, result, obj);
    ...
    let processMessage = (result, obj, done) => {
        ...
        if (Object.keys(obj).length > 0) {
            if (wanumber != null && obj.changes[0].value.hasOwnProperty('contacts') &&
                ...
                contactsPayloadQueue.add(req.body)
                ...
                if (obj.contacts != undefined && obj.messages != undefined) {
                    sendService.checkResponseMessageID(obj.messages[0].id, async (err, result) => {
                        ...
                        if (result === undefined) {
                            ...
                            if (obj.messages[0].type == 'interactive' && obj.messages[0].interactive != undefined) {
                                ...
                                if (obj.messages[0].interactive.type == 'payment') {
                                    console.log('=====================================> ' + obj.messages[0].interactive.type);
                                    paymentTransactionId = obj.messages[0].interactive.payment.transaction_id;
                                    paymentTransactionType = obj.messages[0].interactive.payment.transaction_type;
                                    paymentReferenceId = obj.messages[0].interactive.payment.reference_id;
                                    paymentCurrency = obj.messages[0].interactive.payment.currency;
                                    paymentStatus = obj.messages[0].interactive.payment.status;
                                    paymentTotalAmount = obj.messages[0].interactive.payment.total_amount.value;
                                    paymentTotalAmountOffest = obj.messages[0].interactive.payment.total_amount.offset;
                                    contactsPaymentMessageId = obj.messages[0].id;


                                    if (obj.messages[0].timestamp.length == 10) {
                                        d = new Date(parseInt(obj.messages[0].timestamp) * 1000);
                                        paymentTimeStamp = moment(d).format('YYYY-MM-DD HH:mm:ss');
                                    } else if (obj.messages[0].timestamp.length == 13) {
                                        d = new Date(parseInt(obj.messages[0].timestamp));
                                        paymentTimeStamp = moment(d).format('YYYY-MM-DD HH:mm:ss');
                                    }

                                    contactsMobileNumber = obj.contacts[0].wa_id;
                                    console.log({ paymentTransactionId, paymentTransactionType, paymentReferenceId, paymentCurrency, paymentStatus, paymentTotalAmount, paymentTotalAmountOffest, contactsPaymentMessageId, paymentTimeStamp });
                                    let orderFlag = 0;

                                    let updatePurchaseMasterResult = await sendService.updateDataIntoPurchaseMasterForContactsPayload(paymentReferenceId, paymentTransactionId, 0, parseFloat(paymentTotalAmount / paymentTotalAmountOffest), paymentCurrency, paymentTimeStamp, paymentStatus, orderFlag, contactsPaymentMessageId, contactsMobileNumber);
                                    console.log({ updatePurchaseMasterResult });
                            ...
                            if (obj.messages[0].type == 'button') {
                                ...
                                if (obj.errors !== undefined) {
                                    errorCode = obj.errors[0].code;
                                    errorDesc = obj.errors[0].error_data.details;
                                    readst = 3;
                                    if (obj.messages[0].timestamp.length === 10) {
                                        d = new Date(parseInt(obj.messages[0].timestamp) * 1000);
                                        failedDate = moment(d).format('YYYY-MM-DD HH:mm:ss');
                                    } else if (obj.messages[0].timestamp.length === 13) {
                                        d = new Date(parseInt(obj.messages[0].timestamp));
                                        failedDate = moment(d).format('YYYY-MM-DD HH:mm:ss');
                                    }
                            ...
                            if (obj.messages[0].type == 'unsupported') {
                                console.log('inside UNSUPPORTED MESSAGE TYPE');
                                bodyText = obj.messages[0].errors[0].title;
                                errorCode = obj.messages[0].errors[0].code;
                                errorDesc = obj.messages[0].errors[0].error_data.details;
                                readst = 3;
                                if (obj.messages[0].timestamp.length === 10) {
                                    d = new Date(parseInt(obj.messages[0].timestamp) * 1000);
                                    failedDate = moment(d).format('YYYY-MM-DD HH:mm:ss');
                                ...
                                console.log('=============================================> ' + bodyText + obj.messages[0].type + ' ' + errorCode + ' ' + errorDesc + readst + '' + failedDate);
                            ...
                            if (paymentNameResult != undefined && paymentNameResult[0] != undefined) {
                                console.log('paymentNameResult[0].payment_name1=================> ' + contactno + '==============> ' + paymentNameResult[0].payment_name);
                                paymentName = paymentNameResult[0].payment_name;
                                if (paymentNameResult[0].country_code === 'IN') {
                                    currency = 'INR';
                                }
                            }

                            if (obj.messages[0].type === 'order') {


                                console.log('============================================> inside order block');
                                catalogueId = obj.messages[0].order.catalog_id;
                                productArr = obj.messages[0].order.product_items;
                                console.log(productArr);
                                bodyText = 'CATALOGUE_ORDER_DETAILS';


                                sendService.fetchFlowIdByMobileNumber(contactno, userId, async (err, fetchLastTextMessageResult) => {
                                    // console.log('fetchFlowIdByMobileNumber ============>' + wanumber, obj, contactno, fetchLastTextMessageResult[0].id, userId,fetchLastTextMessageResult[0] != undefined && fetchLastTextMessageResult[0].id != null && fetchLastTextMessageResult[0].id > 0);
                                    if (fetchLastTextMessageResult[0] != undefined && fetchLastTextMessageResult[0].id != null && fetchLastTextMessageResult[0].id > 0) {
                                        let orderDetailsResult = await sendService.orderDetailsPayload(wanumber, obj, contactno, fetchLastTextMessageResult[0].id, fetchLastTextMessageResult[0].current_message_id);
                                            ...
                                            async.waterfall([
                                                ...
                                                function (msg_Type, callback) {
                                                    if (msg_Type == 1) {
                                                        ...
                                                        //     }
                                                        // });
                                                        sendService.fetchFlowIdByMobileNumber(contactno, userId, (err, fetchLastTextMessageResult) => {
                                                            if (fetchLastTextMessageResult[0] != undefined && fetchLastTextMessageResult[0].id != null && fetchLastTextMessageResult[0].id.length > 0) {
                                                                sendService.updateChatAttributes(fetchLastTextMessageResult[0].id, bodyText, (err, result) => {
                                                ...
                                                function (fetchPreviousButtonResult, callback) {
                                                    sendService.unsubkeyword(wanumber, (err, unsubkeywordResult) => {
                                                        //     if (err) {
                                                        //         console.log('unsubkeywordResult error(' + contactno + ')==================>' + err);
                                                        //         //callback(err);
                                                        //     } else {
                                                        //         console.log('unsubkeywordResult result(' + contactno + ')==================>' + JSON.stringify(unsubkeywordResult));
                                                        ...
                                                        if (err) {
                                                            ...
                                                            unsubkeywordResult = unsubkeywordResult[0].stopword;
                                                            unsubkeywordResult = unsubkeywordResult.split(",");
                                                            console.log({ unsubkeywordResult });
                                                            let tempUnsubkeywordResult = null;
                                                            for (let i = 0; i < unsubkeywordResult.length; i++) {
                                                                if (unsubkeywordResult[i].toLowerCase() === bodyText.toLowerCase()) {
                                                                    tempUnsubkeywordResult = bodyText;
                                                                }
                                                            }
                                                            console.log({ tempUnsubkeywordResult });
                                                            console.log('unsubkeywordResult result(' + contactno + ')==================>' + JSON.stringify(tempUnsubkeywordResult));

                                                            callback(null, tempUnsubkeywordResult != undefined && tempUnsubkeywordResult.length > 0 ? tempUnsubkeywordResult : null, fetchPreviousButtonResult);
                                                        }
                                                    });
                                                },
                                                function (unsubkeywordResult, fetchPreviousButtonResult, callback) {
                                                    if (unsubkeywordResult != null && bodyText.toLowerCase() == unsubkeywordResult.toLowerCase()) {
                                                    ...
                                                    else if (bodyText.toLowerCase() == "STOP PROMOTIONS".toLowerCase()) {
                                                        return callback({
                                                            code: 405,
                                                            status: 'FAILED',
                                                            message: 'No Flow Found'
                                                    ...
                                                    if (sessionResult.next_message_id == null) {
                                                        ...
                                                        if (sessionResult.is_node_option == 1) {
                                                            if (sessionResult.is_validator == 1) {
                                                                ...
                                                                if (sessionResult.validator == 'isNationalChequeNumber') {
                                                                    console.log('validator.matches(bodyText, /^\d{6}$/g) : ' + validator.matches(bodyText, /^\d{6}$/g));
                                                                    if (validator.matches(bodyText, /^\d{6}$/g)) {
                                                                        sendService.fetchNextNodeOption(flowid, bodyText, contactno, (err, result) => {
                                                                            console.log('fetchNextNodeOption=====================>' + JSON.stringify(result));
                                                                            if (result[0] != undefined) {
                                                                                let flowTmpPayload = JSON.parse(result[0].node_body);
                                                                ...
                                                                if (sessionResult.validator == 'isNationalZipcode') {
                                                                    if (validator.matches(bodyText, /(^[1-9][0-9]{5})$/g)) {
                                                                        sendService.fetchNextNodeOption(flowid, bodyText, contactno, (err, result) => {
                                                                            console.log('fetchNextNodeOption=====================>' + JSON.stringify(result));
                                                                            if (result[0] != undefined) {
                                                                                let flowTmpPayload = JSON.parse(result[0].node_body);
                                                ...
                                                function (userId, flowid, flowType, flowTmpPayload, next_message_id, current_message_id, sessionResult, placeholder, nextMessageResult, callback) {
                                                    if (sessionResult.session == 0) {
                                                        sendService.setFlowSession(contactno, flowid, userId, next_message_id, current_message_id, flowType, flowTmpPayload, placeholder, (err, result) => {
                                                            // initchatbot related chnages 22082023
                                                            sessionResult.id = result;
                                                            console.log('BEFORE setChatAttributes_1 : ' + JSON.stringify(flowTmpPayload));
                                                            console.log('BEFORE setChatAttributes_2 : ' + JSON.stringify(sessionResult));
                                                            console.log('BEFORE setChatAttributes_3 : ' + result, current_message_id, bodyText, obj.messages[0].id, obj.contacts[0].wa_id, flowTmpPayload);
                                                            sendService.setInitChatAttributes(result, current_message_id, 'bot_init_' + obj.metadata.display_phone_number, bodyText, obj.messages[0].id, obj.contacts[0].wa_id, bodyText, 'init_bot', flowTmpPayload, (err, result) => {
                                                                if (err) {
                                                                    console.log('setInitChatAttributes error=====================>' + err);
                                                                } else {
                                                                    console.log('setInitChatAttributes result=====================>' + result);
                                                                }
                                                ...
                                                if (err) {
                                                    ...
                                                    if (err != null && err.code == 404) {
                                                        callbackService.getReplyMessage(bodyText, wanumber, (err, result) => {
                                                            ...
                                                            if (err) {
                                                                console.log('getReplyMessage error (' + contactno, bodyText, wanumber + ')================>' + err);
                                                            } else {
                                                                console.log('getReplyMessage result (' + contactno, bodyText, wanumber + ')================>' + JSON.stringify(result));
                                                            }
                                                            ...
                                                            let autoReplyMessage = result != undefined && result[0].auto_response != null ? result[0].auto_response : '';
                                                            let unSubscribeKeyword = result != undefined && result[0].stopword != null ? result[0].stopword : '';
                                                            if (unSubscribeKeyword !== null) {
                                                                unSubscribeKeyword = result != undefined && result[0].stopword != null ? result[0].stopword : '';
                                                                unSubscribeKeyword = unSubscribeKeyword.split(",");
                                                                console.log({ unSubscribeKeyword });
                                                                let tempVar = null;
                                                                for (let i = 0; i < unSubscribeKeyword.length; i++) {
                                                                    if (unSubscribeKeyword[i].toLowerCase() === bodyText.toLowerCase()) {
                                                                        unSubscribeKeyword = bodyText;
                                                                    }
                                                                }
                                                                console.log({ unSubscribeKeyword });
                                                            }
                                                            let unSubscribeMessage = result != undefined && result[0].unsubmsg != null ? result[0].unsubmsg : '';
                                                            let reSubscribeKeyword = result != undefined && result[0].resubword != null ? result[0].resubword : '';
                                                            if (reSubscribeKeyword !== null) {
                                                                reSubscribeKeyword = result != undefined && result[0].resubword != null ? result[0].resubword : '';
                                                                reSubscribeKeyword = reSubscribeKeyword.split(",");
                                                                console.log({ reSubscribeKeyword });
                                                                let tempVar = null;
                                                                for (let i = 0; i < reSubscribeKeyword.length; i++) {
                                                                    if (reSubscribeKeyword[i].toLowerCase() === bodyText.toLowerCase()) {
                                                                        reSubscribeKeyword = bodyText;
                                                                    }
                                                                }
                                                                console.log({ reSubscribeKeyword });
                                                            }
                                                            ...
                                                            if (bodyText.toLowerCase() == unSubscribeKeyword.toString().toLowerCase() && unSubscribeMessage.length > 0) {
                                                                callbackService.updateSubscription(contactno, 0, wanumber, null, bodyText, (err, result) => {
                                                                    if (err) {
                                                                        console.log('updateSubscription 1 error(' + contactno + ')==================>' + err);
                                                                    } else {
                                                                        console.log('updateSubscription 1  result(' + contactno + ')================>' + JSON.stringify(result));
                                                                    }
                                                                    ...
                                                                    logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult1) => {
                                                                        if (err) {
                                                                            console.log('logSentByUserMessage1 error(' + contactno + ')==================>' + err);
                                                                        } else {
                                                                            console.log('logSentByUserMessage1 result(' + contactno + ')================>' + JSON.stringify(logSentByUserMessageResult1));
                                                                        }
                                                                ...
                                                                callbackService.updateSubscription(contactno, 1, wanumber, bodyText, null, (err, result) => {
                                                                    if (err) {
                                                                        console.log('updateSubscription(start) error(' + contactno + ')==================>' + err);
                                                                    } else {
                                                                        console.log('updateSubscription(start) result(' + contactno + ')================>' + JSON.stringify(result));
                                                                    }
                                                                    ...
                                                                    logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult2) => {
                                                                        if (err) {
                                                                            console.log('logSentByUserMessage2 error(' + contactno + ')==================>' + err);
                                                                        } else {
                                                                            console.log('logSentByUserMessage2 result(' + contactno + ')================>' + JSON.stringify(logSentByUserMessageResult2));
                                                                        }
                                                                ...
                                                                if (autoResponseFlag == 0) {
                                                                    ...
                                                                    startBot(autoResponseFlowId, userId, contactno, wanumber, isLiveChatActive, bodyText, obj, (err, result) => {
                                                                        if (err) {
                                                                            console.log('startBot error(' + contactno + ')==================>' + err);
                                                                        } else {
                                                                            console.log('startBot result(' + contactno + ')================>' + JSON.stringify(result));
                                                                        }
                                                                ...
                                                                if (autoResponseFlag != 2) {
                                                                    logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult3) => {
                                                                        if (err) {
                                                                            ...
                                                                            console.log('logSentByUserMessage3 error(' + contactno + ')==================>' + err);
                                                                        } else {
                                                                            console.log('logSentByUserMessage3 result(' + contactno + ')================>' + JSON.stringify(logSentByUserMessageResult3));
                                                                        }
                                                    ...
                                                    } else if (err != null && err.code == 405) {
                                                        let unSubscribeMessage = 'You chose to opt out of receiving the marketing message from this business.';

                                                        callbackService.updateSubscription(contactno, 0, wanumber, null, bodyText, (err, result) => {
                                                            if (err) {
                                                                console.log('updateSubscription 1 error==================>' + JSON.stringify(err));
                                                            } else {
                                                            ...
                                                            logSentByUserMessage(userId, obj, isLiveChatActive, bodyText, (err, logSentByUserMessageResult1) => {
                                                                if (err) {
                                                                    console.log('logSentByUserMessage 1 error==================>' + JSON.stringify(err));
                                                                } else {
                                                                    console.log('logSentByUserMessage 1  result================>' + JSON.stringify(logSentByUserMessageResult1));
                                                                }
                                                                done(null, 'Success1 ' + contactno);
                                                            });
                                                        });
            ...
            else if (obj.changes != undefined && obj.changes[0].field == 'template_category_update') {
                businessesPayloadQueue.add(req.body)
                    .then(() => console.log('pushed task'))
                    .catch((e) => console.log('failed to push task'));
                done(null, 'Success');
            }
            else if (obj.changes != undefined && obj.changes[0].field == 'template_performance_metrics') {
                businessesPayloadQueue.add(req.body)
                    .then(() => console.log('pushed task'))
                    .catch((e) => console.log('failed to push task'));
                done(null, 'Success');
    ...
    let sendTemplateMessage = (autoResponseTemplateName, userId, to, wabaNumber, next) => {
        ...
        async.waterfall([
            ...
            (templateMessageResult, templateMessageCallback) => {
                ...
                // changes for AUTORESPONSE MEDIA 
                if (templateMessageResult[0].head_mediatype != undefined && templateMessageResult[0].head_mediatype != '') {
                    let temp_media_type = null;
                    if (templateMessageResult[0].head_mediatype == 0) {
                        temp_media_type = 'document';

                        messagePayload.template.components.push({
                            "type": "header",
                            "parameters": [
                                {
                                    "type": temp_media_type,
                                    [temp_media_type]: {
                                        "link": templateMessageResult[0].mediaurl,
                                        "filename": templateMessageResult[0].mediaurl.substring(templateMessageResult[0].mediaurl.lastIndexOf('/') + 1)
                                    }
                                }
                            ]
                        });
                    }
                    else if (templateMessageResult[0].head_mediatype == 1) {
                        temp_media_type = 'image';
                    ...
                    else if (templateMessageResult[0].head_mediatype == 2) {
                ...
                let tempBodyMsg = templateMessageResult[0].body_message.toString();
                console.log({ tempBodyMsg });
                bodyContent = {
    ...
    let logSentByUserMessage = (userId, obj, isLiveChatActive, bodyText, next) => {
        if (obj.contacts != undefined) {
            ...
            async.waterfall([
                ...
                function (r, done) {
                    callbackService.getResponseRate(waRecipientNo, userId, wanumber, (err, getResponseRateResult) => {
                        if (err) {
                ...
                function (objData, done) {
                    ...
                    sendService.insertMessageInSentMasterAPI(0, null, userId, waRecipientNo, messageText, waMessageId, errorCode, errorDesc, readst, msgType, campaignid, direction, wanumber, wabaId, objData.countrycode, objData.rate, billing, pricing_model, submissiontype, messageText, profileName, failedDate, (err, insertMessageInSentMasterAPIResult) => {
                        if (err) {
        ...
        async.waterfall([
            function (sendMessageCallback) {
                if (msgType == 0) {
                    ...
                    let tmpObj = obj.caption;
                    console.log("tmpObj ---------------- > video ------------", tmpObj)
                    let attrKeyArr = tmpObj.match(/{{(.+?)}}/g);
                    if (attrKeyArr != null && attrKeyArr.length > 0) {
                        //console.log('attrKeyArr=============================>' + attrKeyArr.length);
                        replacePlaceholder(userId, tmpObj, attrKeyArr, to, userProfileName, userMobileNumber, (err, result) => {
                    ...
                    console.log('TEMPLATE MESSAGE===============>' + JSON.stringify(obj));
                    let tmpObj = JSON.stringify(obj.template);
    ...
    let createPayload = async (userId, contactno, flowType, flowTmpPayload, next_message_id, current_message_id, sessionResult, placeholder, nextMessageResult, obj, isLiveChatActive, bodyText, mType, _logflag, next) => {
        ...
        let paymentName = null;
        if (paymentNameResult != undefined && paymentNameResult[0] != undefined) {
            console.log('paymentNameResult[0].payment_name2=================> ' + contactno + '==============> ' + paymentNameResult[0].payment_name);
            paymentName = paymentNameResult[0].payment_name;
            if (paymentNameResult[0].country_code === 'IN') {
                currency = 'INR';
            }
        ...
        // if (flowTmpPayload !== null && flowTmpPayload.product_retailer_id !== undefined) {
        //     paymentOption(userId, flowTmpPayload.product_retailer_id, contactno, (err, paymentOptionResult) => {
        //         if (err) {
        //             console.log(err);
        //         } else {
        //             // console.log({ orderId });
        //             orderId = paymentOptionResult.id;
        //             console.log('====================================> ' + JSON.stringify(paymentOptionResult));
        //         }
        //     });
        // }
        ...
        if (flowTmpPayload !== null && flowTmpPayload.product_retailer_id !== undefined) {
            try {
                paymentOptionResult = await paymentOption(userId, flowTmpPayload.product_retailer_id, contactno);
                console.log('====================================> ' + JSON.stringify(paymentOptionResult));
                if (paymentOptionResult !== undefined) {
                    orderId = paymentOptionResult.trnValue;
                    rzpOrderId = paymentOptionResult.orderId;
                    console.log({ orderId });
                }
        ...
        async.waterfall([
            ...
            (r, createPayloadCallback) => {
                ...
                else if (flowType == 'Catalogue') {
                    try {
                        let data = {};

                        if (flowTmpPayload.product_type === 'Single') {
                            ...
                            data.interactive = {
                                ...
                                "action": {
                                    "catalog_id": flowTmpPayload.catalog_id,
                                    "product_retailer_id": flowTmpPayload.product_retailer_id
                                }
                            };
                        }
                        else if (flowTmpPayload.product_type === 'Multiple') {
                            sectionCount = flowTmpPayload.section_count;
                            ...
                            if (parseInt(sectionCount) > 0) {
                                for (let i = 0; i < sectionCount.length; i++) {
                                    Object.keys(flowTmpPayload.sections).forEach((key, index) => {
                                        console.log({ key });
                                        if (isNaN(key) === false) {
                                            sectionText = flowTmpPayload.sections[key].section_text;
                                            ...
                                            value1 = flowTmpPayload.sections[key].retailer_ids;
                                            let productRetailerId = new Array();
                                            productRetailerId = value1.split(",");
                                            console.log('Catalogue ====================================> ' + productRetailerId);
                                            // console.log({ productRetailerId });

                                            for (let i = 0; i < productRetailerId.length; i++) {
                                                data.interactive.action.sections[index].product_items.push({
                                                    "product_retailer_id": productRetailerId[i]
                                                });
                                            }
                                        }
                        ...
                        createPayloadCallback(null, flowTmpPayload, data.interactive, sessionResult, next_message_id, current_message_id, 9, flowType, placeholder, nextMessageResult);

                    } catch (err) {
                        console.log(err);
                    }
                }
                else if (flowType === 'WhatsappPayment') {
                    sendService.getOrderDetailsData(contactno, sessionResult.id, (err, orderResult) => {
                        if (err) {
                            console.log(' getOrderDetailsData err ===================================>' + err);
                        }
                        else {
                            console.log(' getOrderDetailsData result ===================================>' + JSON.stringify(orderResult));


                            // console.log('============================================> inside order block');
                            let payload = JSON.parse(orderResult[0].order_details_payload);


                            catalogueId = payload.messages[0].order.catalog_id;
                            productArr = payload.messages[0].order.product_items;
                            console.log(productArr);
                            bodyText = 'CATALOGUE_ORDER_DETAILS';

                            console.log({ payload, catalogueId, productArr });

                            let botMessagePayload = {};
                            let catalogueOrderId = null;
                            ...
                            sendService.getTaxData(userId, catalogueId, (err, taxData) => {
                                if (err) {
                                    console.log('getTaxData err ==========================> ' + err);
                                }
                                else {
                                    console.log('getTaxData result ==========================> ' + taxData);
                                    ...
                                    paymentOptionForCatalogue(userId, parseInt(totalAmount), contactno, (err, paymentOptionForCatalogueResult) => {
                                        if (err) {
                                            console.log(err);
                                        } else {
                                            catalogueOrderId = paymentOptionForCatalogueResult.orderId;
                                            console.log({ catalogueOrderId });
                                            console.log('====================================> ' + JSON.stringify(paymentOptionForCatalogueResult));

                                            // trnValue = 'test978654321000000';
                                            trnValue = paymentOptionForCatalogueResult.trnValue;
                                            // catalogueOrderId = '123456';
                                            let parameters = {
                                                // "reference_id": paymentOptionForCatalogueResult.trnValue,
                                                "reference_id": trnValue,
                                                "type": "digital-goods",
                                                "currency": "INR",
                                                "total_amount": {
                                                    "value": parseInt(totalAmount) * 100,
                                                    "offset": 100
                                                },
                                                "payment_type": "upi",
                                                "payment_configuration": paymentName,
                                                "order": {
                                                    ...
                                                    "items": [],
                                                    "subtotal": {
                                                        "value": parseInt(subTotal) * 100,
                                                        "offset": 100
                                                    },
                                                    "tax": {
                                                        "value": parseInt(tax) * 100,
                                                        "offset": 100
                                                    }
                                            ...
                                            sendService.fetchAccessToken((err, tokenResult) => {
                                                if (err) {
                                                    console.log('fetchAccessToken err ====================> ' + err);
                                                }
                                                else {
                                                    console.log('fetchAccessToken err ====================> ' + tokenResult);
                                                    let accessToken = tokenResult[0].VALUE;
                                                        ...
                                                        .then(async function (response) {
                                                            ...
                                                            for (let j = 0; j < tempProductArr.length; j++) {
                                                                for (let i = 0; i < productArr.length; i++) {
                                                                    ...
                                                                    if (tempProductArr[j].retailer_id === productArr[i].product_retailer_id) {
                                                                        ...
                                                                        let items = {
                                                                            "retailer_id": productArr[i].product_retailer_id,
                                                                            "product_id": tempProductArr[j].id,
                                                                            "name": tempProductArr[j].name.toString().substring(0, 50),
                                                                            "amount": {
                                                                                "value": parseInt(productArr[i].item_price) * 100,
                                                                                "offset": 100
                                                                            },
                                                                            "quantity": parseInt(productArr[i].quantity)

                                                                        };

                                                                        // console.log('ITEMS ===================>' + JSON.stringify(items));
                                                                        parameters.order.items.push(items);
                                                            ...
                                                            botMessagePayload = {
                                                                ...
                                                                "interactive": {
                                                                    ...
                                                                    //     "type": "image",
                                                                    //     "image": {
                                                                    //         "link": "https://wpstaging.pinnacle.in/assets/flowbuilder/145/299.jpg"
                                                                    //     }
                                                                    // },
                                                                    // "footer": {
                                                                    //     "text": "www.pinnacle.in"
                                                                    // },
                                                                    "action": {
                                                                        "name": "review_and_pay",
                                                            ...
                                                            messageTypeNumber = 9;
                                                            console.log('CATALOGUE ORDER DETAILS ============================> ' + JSON.stringify(botMessagePayload));
                                                            // botUtils.axiosPostData(phonenoid, botMessagePayload, userId, contactno, messageTypeNumber, wanumber);
                                                            ...
                                                            // rows6 = await dbpool.query(query6, [sessionResult.id]);
                                                            ...
                                                            rows1 = await dbpool.query(query1, [contactno, 0]);
                                                            ...
                                                            query8 = "SELECT customer_details from ezb_non_catalog_purchase_master WHERE userid = ? AND mobileno =? AND catalog_type = ? ORDER BY id DESC LIMIT 1";
                                                            rows8 = await dbpool.query(query8, [userId, contactno, 1]);
                                                            ...
                                                            // let insertDataIntoPurchaseMasterResult = await sendService.insertDataIntoPurchaseMaster(userId, contactno, 0, productRetailerIds, paymentOptionForCatalogueResult.trnValue, catalogueOrderId, obj.messages[0].id, paymentReferenceId, transactionId, transactionType, totalAmount, currency, orderStatus, orderFlag, catalogueType, productImage);

                                                            console.log(JSON.stringify(productRetailerIds));

                                                            console.log('CATALOGUE WHATSAPP PAYMENT =========================>' + contactno + JSON.stringify(userId, contactno, JSON.stringify(productRetailerIds), trnValue, catalogueOrderId, obj.messages[0].id, paymentReferenceId, transactionId, transactionType, totalAmount, currency, orderStatus, orderFlag, catalogueType, productImage, JSON.stringify(userDetails)));


                                                            let insertDataIntoPurchaseMasterResult = await sendService.insertDataIntoPurchaseMaster(userId, contactno, 0, JSON.stringify(productRetailerIds), trnValue, catalogueOrderId, obj.messages[0].id, paymentReferenceId, transactionId, transactionType, totalAmount, currency, orderStatus, orderFlag, catalogueType, productImage, JSON.stringify(userDetails));
                                                            console.log({ insertDataIntoPurchaseMasterResult });

                                                            let wapayContextId = await fetchWAPAYContextId(contactno);
                                                            console.log({ wapayContextId });

                                                            await sendService.updateOrderDetailsData(wapayContextId, contactno, sessionResult.id, insertDataIntoPurchaseMasterResult.insertId);
                ...
                else if (flowType == "WhatsappTemplate") {
                    console.log('typenode ================> ' + flowType + contactno);
                    sendService.getTemplateData(flowTmpPayload.template_title, userId, (err, getTemplateDataResult) => {
                        if (err) {
                            console.log('getTemplateDataResult ============================================> ' + err + contactno);
                        }
                        else {
                            console.log('getTemplateDataResult ============================================> ' + JSON.stringify(getTemplateDataResult) + contactno);


                            let templateType = flowTmpPayload.template_type;
                            let templateTitle = flowTmpPayload.template_title;
                            let headerPlaceholder = flowTmpPayload.header_placeholder;
                            let bodyPlaceholder = flowTmpPayload.body_placeholder;
                            let dynamicUrl = flowTmpPayload.dynamic_url;
                            let mediaUrl = flowTmpPayload.media_url;


                            console.log({ templateType, templateTitle, headerPlaceholder, bodyPlaceholder, dynamicUrl, mediaUrl });

                            let botMessagePayload = {};

                            botMessagePayload = {
                                "template": {
                                    "name": getTemplateDataResult[0].temptitle,
                                    "language": {
                                        "code": getTemplateDataResult[0].langcode
                                    },
                                    "components": []
                                }
                            };

                            if (getTemplateDataResult[0].placeholders !== null && getTemplateDataResult[0].placeholders !== '') {
                                let temp = flowTmpPayload.body_placeholder !== null ? flowTmpPayload.body_placeholder.split(',') : null;
                                console.log({ temp });


                                if (getTemplateDataResult[0].head_temptype === "0") {
                                    console.log('TEXT TEMPLATE =======================================================> ' + ' ' + contactno + ' ' + getTemplateDataResult[0].head_temptype);
                                    botMessagePayload = botMessagePayload;

                                    // console.log({ botMessagePayload });
                                }
                                else if (getTemplateDataResult[0].head_temptype === "1") {
                                    console.log('HEAD_TEMP_TYPE =======================================================> ' + ' ' + contactno + ' ' + getTemplateDataResult[0].head_temptype);

                                    if (getTemplateDataResult[0].head_mediatype === "0") {
                                        console.log('DOCUMENT TEMPLATE =======================================================> ' + ' ' + contactno + ' ' + getTemplateDataResult[0].head_temptype + '' + getTemplateDataResult[0].head_mediatype);

                                        botMessagePayload.template.components.push({
                                            "type": "header",
                                            "parameters": [
                                                {
                                                    "type": "document",
                                    ...
                                    else if (getTemplateDataResult[0].head_mediatype === "1") {

                                        console.log('IMAGE TEMPLATE =======================================================> ' + ' ' + contactno + ' ' + getTemplateDataResult[0].head_temptype + '' + getTemplateDataResult[0].head_mediatype);
                                    ...
                                    else if (getTemplateDataResult[0].head_mediatype === "2") {
                                        console.log('VIDEO TEMPLATE =======================================================> ' + ' ' + contactno + ' ' + getTemplateDataResult[0].head_temptype + '' + getTemplateDataResult[0].head_mediatype);
                            ...
                            if (getTemplateDataResult[0].button_option === "0") {
                                // console.log(getTemplateDataResult[0].button_option_string);

                                let buttonString = JSON.parse(getTemplateDataResult[0].button_option_string);
                                console.log({ buttonString });
                                // console.log(buttonString[0]);
                                // console.log(buttonString[1]);



                                if (flowTmpPayload.dynamic_url !== null) {
                                    console.log(getTemplateDataResult[0].button_option + ' =======================> DYNAMIC URL' + contactno);
                                    // for (let i = 0; i < 3; i++) {

                                    if (buttonString[0].hasOwnProperty('visit_website')) {
                                        index = 0;
                                    }
                                    else {
                                        index = 1;
                                    ...
                                    botMessagePayload.template.components.push({
                                        ...
                                        "parameters": [
                                            {
                                                "type": "text",
                                                "text": flowTmpPayload.dynamic_url
                                            }
                                        ]
                                    });
                                    ...
                                    console.log(getTemplateDataResult[0].button_option + ' =======================> STATIC URL' + contactno);
                                    botMessagePayload = botMessagePayload;
                                }

                            }
                            else if (getTemplateDataResult[0].button_option === "1") {
                                console.log(getTemplateDataResult[0].button_option + ' =======================> QUICK REPLY' + contactno);
                                // for (let i = 0; i < 3; i++) {
                                //     botMessagePayload.template.components.push({
                                //         "type": "button",
                            ...
                            console.log(botMessagePayload + ' ' + contactno);
                            flowTmpPayload.template_data = getTemplateDataResult;
                            console.log({ botMessagePayload });
                            createPayloadCallback(null, flowTmpPayload, botMessagePayload, sessionResult, next_message_id, current_message_id, 8, flowType, placeholder, nextMessageResult);
            ...
            (flowTmpPayload, botMessagePayload, sessionResult, next_message_id, current_message_id, flowType, typeNode, placeholder, nextMessageResult, createPayloadCallback) => {
                ...
                if (typeNode == "Condition") {
                    ...
                    if (typeNode == "List") {
                        ...
                        is_node_option = 0;
                    ...
                    else if (typeNode == 'WhatsappTemplate') {
                            // for (let i = 0; i < botMessagePayload.template.components.length; i++) {
                            //     if (botMessagePayload.template.components !== null && botMessagePayload.template.components[i].type === 'button') {
                            //         if (botMessagePayload.template.components[i].sub_type === 'quick_reply') {
                            //             is_node_option = 1;
                            //         } else if (botMessagePayload.template.components[i].sub_type === 'URL') {
                            //             is_node_option = 0;
                            //             // next_message_id = 0; // session_end for call-to-action type 
                            //         }
                            //     } else {
                            //         is_node_option = 0;
                            //     }
                            ...
                            if (flowTmpPayload.template_data[0].button_option_string.includes('quick_reply')) {
                                is_node_option = 1;
                            } else {
                                is_node_option = 0;
                            }
                        console.log({ is_node_option });
                    }
                    ...
                    if (typeNode != "Message") {
                        sendMessage(userId, result.payload, contactno, flowType, async (err, sendMessageResult) => {
                            console.log({ contactno, sendMessageResult, result, typeNode });
                            if (err) {
                                console.log('Send message error====================>' + err);
                                console.log(err);
                            } else {
                                ...
                                if (typeNode === 'CataloguePayment') {
                                    console.log("Product Retailer Id : " + flowTmpPayload.product_retailer_id, contactno);
                                    ...
                                    let transactionId = 0;
                                    let transactionType = 0;
                                    let totalAmount = flowTmpPayload.total_amount;
                                    let currency = "INR";
                                    let orderStatus = "pending";
                                    let orderFlag = 0;
                                    let productImage = flowTmpPayload.header_media_url;

                                    console.log({
                                        userId, contactno, nonCatalogueId,
                                        productRetailerId, orderId, rzpOrderId, sendMessageResult, paymentReferenceId, transactionId, transactionType, totalAmount,
                                        currency, orderStatus, orderFlag, productImage
                                    });

                                    let insertDataIntoPurchaseMasterResult = await sendService.insertDataIntoPurchaseMasterEnd(userId, contactno, nonCatalogueId,
                                        productRetailerId, orderId, rzpOrderId, sendMessageResult, paymentReferenceId, transactionId, transactionType, totalAmount,
                                        currency, orderStatus, orderFlag, 0, productImage);
                                    console.log({ insertDataIntoPurchaseMasterResult });
                ...
                axios(config)
                    .then(function (response) {
                        ...
                        let customCallbackPayload = {
                            ...
                            'response_data': response.data,
                            'responseTime': diffTime + ' ms',
                            'callback_wanumber': callback_wanumber,
                            'response_status': response.status
                        };

                        // customCallbackPayloadQueue.add(customCallbackPayload)
                        //     .then(() => console.log('pushed task'))
                        //     .catch((e) => console.log('failed to push task'));
                    ...
                    .catch(async function (error) {
                        ...
                        let customCallbackPayload = {
                            ...
                            'responseTime': diffTime + ' ms',
                            'callback_wanumber': callback_wanumber,
                            'response_status': error.response != undefined ? error.response.status : null
                        };
                        ...
                        // let sts = null;
                        // let readst = null;
                        // console.log('============================================> ' + JSON.stringify(payload), msgId, callback_wanumber);
                        // if (payload.object === 'whatsapp_business_account' && payload.entry[0].changes[0].value.hasOwnProperty('statuses')) {
                        //     console.log('inside failed statuses block ');
                        //     msgId = payload.entry[0].changes[0].value.statuses[0].id;
                        //     sts = payload.entry[0].changes[0].value.statuses[0].status;
                        //     payloadType = 1;
                        //     retryCount = 0;
                        //     if (sts === 'sent') {
                        //         readst = 0;
                        //         let msgIdCount = await sendService.getMessageIdCountFromFailedCallbackLogs(msgId, readst);
                        //         console.log({ msgIdCount });
                        //         if (msgIdCount === 0) {
                        //             let result = await sendService.insertFailedCallbackLogs(payload, msgId, readst, callback_wanumber, url, retryCount, payloadType);
                        //             console.log({ result });
                        //         }
                        //         else if (msgIdCount > 0) {
                        //             console.log('Message Id already exits');
                        ...
                        //     else {
                        //         readst = null;
                        //         console.log('no status payload found');
                        //     }

                        // }
                        // else if (payload.object === 'whatsapp_business_account' && payload.entry[0].changes[0].value.hasOwnProperty('contacts')) {
                        //     console.log('inside failed contacts block ');
                        //     readst = null;
                        ...
                        // }
                        // else {
                        //     console.log('No statuses or contacts payload found');
                        // }
        ...
        async.waterfall([
            function (sendEmailCallback) {
                ...
                sendService.fetchEmailData(userId, wa_id, (err, fetchEmailDataResult) => {
                    console.log('fetchEmailData=======================>' + JSON.stringify(fetchEmailDataResult) + ", wa_id : " + wa_id + ", result.current_message_id : " + result.current_message_id + ", userId : " + userId);
                    if (err) {
                            ...
                            if (err) {
                                ...
                                sendMessage(userId, orderDetailsPayload, obj.contacts[0].wa_id, 9, async (err, sendMessageResult) => {
                                    if (err) {
                                        ...
                                        let totalAmount = executeWebhookResult.data.productamount * executeWebhookResult.data.productquantity;
                                        let paymentReferenceId = 0;
                                        let transactionId = 0;
                                        let transactionType = 0;
                                        let currency = "INR";
                                        let orderStatus = "pending";
                                        let orderFlag = 0;
                                        let productImage = executeWebhookResult.data.productimageurl;

                                        let insertDataIntoPurchaseMasterResult = await sendService.insertDataIntoPurchaseMasterEnd(userId, obj.contacts[0].wa_id, nonCatalogueId,
                                            productRetailerId, orderId, orderId, sendMessageResult, paymentReferenceId, transactionId, transactionType, totalAmount,
                                            currency, orderStatus, orderFlag, 0, productImage);
