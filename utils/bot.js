...
module.exports = {
    ...
    callWhatsAppApiPUT: function (instanceUrl, api, data, httpMethod, requestType, apiHeaders) {
        var url = instanceUrl + api;
        var httpMethod = parseInt(httpMethod);
        var requestType = parseInt(requestType);
        var postData = {};
    ...
    axiosPostData: async function (phoneNumberId, data, userId, mobileNumber, messageTypeNumber, wanumber, countryCode) {
        let result1 = await sendService.customCallbackPayloadData(wanumber);
        let url = result1[0].waurl;
        let msgSettingId = result1[0].wa_msg_setting_id;
        console.log({ url });

        let result2 = await sendService.fetchSystemAccessToken();
        let accessToken = result2[0].value;
        ...
        try {
            ...
            // console.log({ response });
            let msgId1 = response.data.messages[0].id;
            // console.log({ msgId1 });
            let insertResult = await sendService.insertPaymentMessageInSentMaster(userId, mobileNumber, JSON.stringify(data), msgId1, messageTypeNumber, 0, wanumber, msgSettingId, countryCode);
