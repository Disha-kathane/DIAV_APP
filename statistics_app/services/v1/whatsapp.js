...
const getHealth = async (waAuthToken, instanceUrl, api, next) => {
    try {
        ...
        //console.log('wagetHealthResult', wagetHealthResult);
...
const getQualitySignalStatus = async (id, access_token, fields, next) => {
    const instanceUrl = 'https://graph.facebook.com';
    const api = '/v11.0/' + id;
    const httpMethod = 0;
    const requestType = 0;
...
const getSettingApi = async (waAuthToken, instanceUrl, api, next) => {
    try {
        const httpMethod = 0;
        const requestType = 0;
        const apiHeaders = [{
        ...
        const getWebhookResult = await botUtils.callWhatsAppApi(instanceUrl, api, null, httpMethod, requestType, apiHeaders);
        //console.log('wagetHealthResult', wagetHealthResult);
        next(null, getWebhookResult);
    } catch (err) {
        getSettingApi
        console.log(err);
        next(err);
...
const updateSettingApi = async (waAuthToken, instanceUrl, api, payload, next) => {
    try {
        const httpMethod = 1;
        const requestType = 1;
        const apiHeaders = [{
        ...
        const setWebhookResult = await botUtils.callWhatsAppApiPatch(instanceUrl, api, payload, httpMethod, requestType, apiHeaders);

        next(null, setWebhookResult);
    } catch (err) {
        console.log(err);
        next(err);
    }
...
const webhookSettingApi = async (waAuthToken, instanceUrl, api, payload, next) => {
    try {
        const httpMethod = 1;
        const requestType = 1;
        const apiHeaders = [{
        ...
        const WebhookResult = await botUtils.callWhatsAppApiPatch(instanceUrl, api, payload, httpMethod, requestType, apiHeaders);

        next(null, WebhookResult);
    } catch (err) {
        // console.log(err);
        next(err);
    }
...
const getConversationMetrics = async (fields, access_token, wabaId, next) => {
    const instanceUrl = 'https://graph.facebook.com';
    const api = '/v12.0/' + wabaId;
    const httpMethod = 0;
    const requestType = 0;
    const apiHeaders = [];
    const objData = {
