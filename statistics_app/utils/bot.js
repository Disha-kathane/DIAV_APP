...
module.exports = {
    ...
    isMobileInternational1: function (number) {
        if (typeof number == 'undefined' || number.length == 0) {
            return true;
        }
    ...
    callWhatsAppApi: function (instanceUrl, api, data, httpMethod, requestType, apiHeaders) {
        ...
        console.log("inside whats app api",{url})
    ...
    callWhatsAppApiPatch: function (instanceUrl, api, data, httpMethod, requestType, apiHeaders) {
        var url = instanceUrl + api;
        var httpMethod = parseInt(httpMethod);
        var requestType = parseInt(requestType);
        var postData = {};
        ...
        return new Promise(function (resolve, reject) {
            ...
            try {
                if (httpMethod == 1) { //POST
                    instance.patch(url, postData).then(function (response) {
                        console.log(response);
                        resolve(response.data);
                    }).catch(function (error) {
                        reject(error);
