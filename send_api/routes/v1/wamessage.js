...
const sendMessageController = require('../../controller/v1/sendMessage');
const sendClick2CallController = require('../../controller/v1/sendClick2CallController');
...
const MediaCloudController = require('../../controller/v1/mediacloud');
const getMediaCloudController = require('../../controller/v1/getmediacloud');
const fetchWabaController = require('../../controller/v1/fetchWabaController');
const fetchTemplateController = require('../../controller/v1/fetchTemplateController');
const bulkmessagingController = require('../../controller/v1/bulkmessaging');
const registerController = require('../../controller/v1/register');
const loginController = require('../../controller/v1/login');
const sendEncryptionController = require('../../controller/v1/sendEncryption');
const encryptionController = require('../../controller/v1/encryption');
...
module.exports = (upload) => {
    ...
    const sendClick2CallOpts = {
        handler: sendClick2CallController
    };

    const mediaCloudOpts = {
        handler: MediaCloudController
    };

    const getmediacloudOpts = {
        handler: getMediaCloudController
    };

    const bulkMsgSendingOpts = {
        handler: bulkmessagingController
    };
    ...
    const sendEncryptionOpts = {
        handler: sendEncryptionController
    };
    ...
    // For cloud as well as for premises 06/06/2022 added by 
    const sendCloudApi = async (req, res) => {
        try {
            let userId = null;
            let wanumber = null;
            ...
            let category = null;
            let categoryId = 0;
            console.log('send API Payload : ' + JSON.stringify(messageObj));
            ...
            let wabaInfo = (result1, done) => {
                ...
                sendService.checkWanumber(wabanumber, (err, result) => {
                    if (err) {
                        // console.log('CHECKWANUMBER ERROR=====================>' + err, wabanumber);
                        console.log(err);
                    } else {
                        // console.log('CHECKWANUMBER RESULT=====================>' + JSON.stringify(result), wabanumber);
                    }
                    ...
                    if (result.length > 0) {
                        ...
                        if (phone_number_id != '' && (phone_number_id != null && phone_number_id != undefined)) {
                            // console.log('I am here 1 : ' + phone_number_id);
                            setflag = 1;
                            sendService.getSystemAccessToken((err, result) => {
                                ...
                                sendService.getSpecificWabaInfo(result1[0].userid, wabanumber, (err, getSpecificWabaInfoResult) => {
                                    if (err) {
                                        ...
                                        getSpecificWabaInfoResult.authtoken = SystemAccessToken;
                            ...
                            // console.log('I am here 2 : ' + phone_number_id);
                            sendService.getSpecificWabaInfo(result1[0].userid, wabanumber, (err, getSpecificWabaInfoResult) => {
                                if (err) {
                                    return done({
                                        code: err.errno,
            ...
            let checkSubscription = (messagePayload, wabaInfoResult, msgType, done) => {
                // console.log("inside send checkSubscription =============" + JSON.stringify(wabaInfoResult))
            ...
            let sendMessage = (messagePayload, wabaInfoResult, msgType, done) => {
                ...
                // console.log("inside send message =============" + JSON.stringify(messagePayload))
                if (setflag == 0) {
                    ...
                    // console.log("inside cloud", messagePayload);
                    let direction = 1;
                    // let api = '/v1/messages';
                    let httpMethod = 1;
                    let requestType = 1;
                    ...
                    let apiHeaders = [{
                        'headerName': 'Authorization',
                        'headerVal': 'Bearer ' + wabaInfoResult.authtoken
                    }, {
                        'headerName': 'content-length',
                        'headerVal': contentLength
                    }];
                    // console.log('MESSAGE_PAYLOAD=======================>' + JSON.stringify(messagePayload));
                    // console.log('MESSAGE_PAYLOAD authtoken=======================>' + wabaInfoResult.authtoken);
                    // console.log('MESSAGE_PAYLOAD httpMethod=======================>' + httpMethod);
                    // console.log('MESSAGE_PAYLOAD requestType=======================>' + requestType);
                    // console.log('MESSAGE_PAYLOAD apiHeaders=======================>' + apiHeaders);

                    botUtils.callWhatsAppApi1(wabaInfoResult[0].waurl, messagePayload, httpMethod, requestType, apiHeaders).then((response) => {
                        console.log('SEND_API_RESPONSE' + JSON.stringify(response));
                        if (typeof response.messages != undefined) {
                            let errorCode = null;
                            let errorDesc = null;
                        ...
                        console.log('SEND_API_RESPONSE ERROR : ' + err);
                        // console.log('CLOUD_ERROR====================>' + messagePayload.to + '===============>' + JSON.stringify(err.response.data.error));
                        // console.log(err);
                        //added for cloud non whatsapp number on 27/6/2022

                        if (err.response != undefined && err.response.data != undefined && (err.response.data.error.code == 131009 || err.response.data.error.code == 100)) {
            ...
            console.log('SENDCLOUDAPI ERROR===============================>' + error);
    ...
    //                 if (err) {
    //                     console.log('getSpecificWabaInfo error====================>' + JSON.stringify(err));
    //                     return done({
    //                         code: err.errno,
    //                         status: 'FAILED',
    //                         message: err.code,
    ...
    //                 else {
    //                     if (result.length > 0) {
    //                         // console.log('getSpecificWabaInfo result====================>' + JSON.stringify(result));
    //                         done(null, result);
    //                     }
    //                     else {
    //                         return done({
    //                             code: 'WA037',
    //                             status: 'FAILED',
    //                             message: 'from number is invalid or do not exists',
    //                             data: {}
    //                         });
    //                     }
    //                 }
    ...
    //                         bodyContent = {
    //                             "body": "<a href='" + messageObj.message.id + "'>Media</a>"
    //                         };
    //                     }
    ...
    //                     bodyContent = {
    //                         "body": {
    //                             "latitude": messageObj.message.latitude, "longitude": messageObj.message.longitude
    //                         }
    //                     };
    ...
    //         let checkSubscription = (messagePayload, wabaInfoResult, msgType, done) => {
    //             // console.log("check subcription ", json.stringify(messagePayload))
    //             sendService.checkSubscription(messagePayload.to, wabaInfoResult[0].wanumber, (err, result) => {
    //                 if (err) {
    //                     return done({
    //                         code: err.errno,
    ...
    //                     sendService.insertMessageInSentMasterAPI(0, 0, userId, messagePayload.to, messagePayload, waMessageId, msgType, 0, wabaInfoResult[0].wanumber, wabaInfoResult[0].wa_msg_setting_id, direction, errorCode, errorDesc, messageObj, bodyContent, countryCodeNumeric, (error, result) => {
    //                         if (error) {
    //                             errorLogger.error("**********************************************************");
    //                             errorLogger.error("REQUEST_BODY: " + JSON.stringify(req.body));
    //                             errorLogger.error("WHATSAPP_PAYLOAD: " + JSON.stringify(messagePayload));
    ...
    //                 // console.log(err.response.data.errors[0].details);

    //                 return done({
    //                     code: 'WA100',
    //                     status: 'FAILED',
    //                     message: err.response != undefined ? err.response.data.errors[0].details : err,
    //                     data: {}
    //                 });
    //             });
    //         };
        ...
        try {
            ...
            let checkOptin = (wabaInfo, done) => {
                sendService.checkOptinContactStatus_Mod(from, to, (err, result) => {
                    if (err) {
                        console.log('checkOptinContact(' + to + ') err:' + JSON.stringify(err));
                        return done({
                            code: err.errno,
                            status: 'FAILED',
                            message: err.code,
                        ...
                        if (result.length > 0) {
                            ...
                            // console.log('checkOptinContact(' + to + ') result :' + JSON.stringify(result[0].wastatus));
            ...
            let doOptin = (wabaInfo, done) => {
                ...
                botUtils.callWhatsAppApi(wabaInfo[0].waurl, api, objData, httpMethod, requestType, apiHeaders).then((response) => {
                    // console.log('Optin Response(' + to + ')====================>' + JSON.stringify(response));
                    ...
                    // console.log('Optin Error(' + to + ')====================>' + JSON.stringify(err));
                    // console.log(err);
                    return done({
                        code: 'WA100',
                        status: 'FAILED',
    ...
    const fetchWabaOpts = {
        handler: fetchWabaController
    };

    const fetchTemplateOpts = {
        handler: fetchTemplateController
    };
    ...
    return (fastify, opts, done) => {
        ...
        // fastify.post('/sendcloud', sendSchema, sendCloudApi);

        fastify.post('/send', sendSchema, sendCloudApi);
        // fastify.post('/sendcloud', sendSchema, sendCloudApi);

        fastify.post('/media', mediaOpts);
        fastify.get('/media/:mediaid', getMediaOpts);
        fastify.post('/sendMessage', sendMessageOpts);
        fastify.post('/mediacloud', mediaCloudOpts);
        fastify.get('/getmediacloud/:mediaid', getmediacloudOpts);
        fastify.post('/sendClick2CallNotification', sendClick2CallOpts);
        fastify.post('/fetchWaba', fetchWabaOpts);
        fastify.post('/fetchTemplate', fetchTemplateOpts);
        fastify.post('/bulksendMsg', bulkMsgSendingOpts);
        fastify.post('/register', registerOpts);
        fastify.post('/login', loginOpts);
        fastify.post('/sendRequest', sendEncryptionOpts);
        fastify.post('/encrypt', encryptionOpts);
        done();
    };
};