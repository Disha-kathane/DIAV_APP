...
module.exports = cron.schedule('*/10 * * * * *', async function () {
    try {
        let fetchMessages = (done) => {
            sendService.fetchScheduledMessages(function (err, result) {
                if (err) {
                    console.log(err);
                    return done({
            ...
            for (let index = 0; index < msgResult.length; index++) {
                ...
                sendService.checkSubscription(msgResult[index].mobileno, msgResult[index].contactno, (err, result) => {
                    if (result == 0) {
                        ...
                        async.waterfall([
                            function (done) {
                                sendService.insertMessageInSentMaster(msgResult[index].id, msgResult[index].botid, msgResult[index].userid, msgResult[index].mobileno, JSON.stringify(objMsg), waMessageId, msgResult[index].messagetype, msgResult[index].campaignid, msgResult[index].contactno, msgResult[index].msg_setting_id, direction, errorCode, errorDesc, bodyContent, function (err, result) {
                                    done(err, result);
                                });
                            },
